...
REG_VAR( r_render_friends_name		, false		, 0 ); //AlexRedd:: A way to show player names in same group/clan
...
REG_VAR(g_PinCodeFirstStart, true, 0);

REG_VAR( r_width				, 1024		, VF_SAVE );		// width window
REG_VAR( r_height				, 768		, VF_SAVE );		// height window

REG_VAR_C( r_overall_quality		, 2			, 1, 6, VF_SAVE | VF_CONSTRAINT );
...
REG_VAR( g_FastLoading			, true			, VF_SAVE );// disable fastload opt

REG_VAR( r_chat_message_sound   , true          , VF_SAVE ); //ChatSound
REG_VAR( r_increase_fps			, true			, VF_SAVE ); // increase fps opt
REG_VAR( r_disable_grass		, false			, VF_SAVE ); //Disable grass opt
REG_VAR( r_disable_jump_sound	, true			, VF_SAVE ); //Disable jump sound
REG_VAR( r_enable_alpha_sounds	, false			, VF_SAVE ); // Alpha music, sounds
REG_VAR( r_enable_killfeed_messages, true		, VF_SAVE ); // KillFeed messages
REG_VAR( r_enable_killstreak_sounds, true		, VF_SAVE ); // KillSreak sounds enable
REG_VAR( r_killstreak_sounds		, 0			, VF_SAVE ); // KillSreak sounds pack (0-crossfire, 1-quake, 2-Duke)

REG_VAR_C( r_crosshaire_mode		, 0			, 0, 7, VF_SAVE | VF_CONSTRAINT );// Crosshire mode
REG_VAR_C( r_crosshaire_color		, 0			, 0, 3, VF_SAVE | VF_CONSTRAINT ); // Crosshaire color (0 ="white", 1 ="red", 2 ="blue", 3 ="green")
REG_VAR_C( r_highlight_mode			, 0			, 0, 6, VF_SAVE | VF_CONSTRAINT );// Highlight mode	

REG_VAR_C( r_mesh_quality			, 3			, 1, 3, VF_SAVE | VF_CONSTRAINT );
REG_VAR_C( r_texture_quality		, 2			, 1, 4, VF_SAVE | VF_CONSTRAINT );
REG_VAR_C( r_terrain_quality		, 2			, 1, 4, VF_SAVE | VF_CONSTRAINT );
...
REG_VAR( g_serverip,			"26.224.36.102",		0 );
REG_VAR( g_api_ip,				"26.224.36.102",		0 );
