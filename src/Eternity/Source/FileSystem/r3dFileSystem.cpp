...
DWORD WINAPI CompressFileInThread(LPVOID param) 
{
    ThreadData* data = static_cast<ThreadData*>(param);
    r3dFSCompress compress;

    for (int i = data->startIdx; i < data->endIdx; ++i) {
        r3dFS_FileEntry& fe = *data->fileSystem->fl_.files_[i];

        float cperc;
        cperc = (float)(*data->stat_size) / (*data->stat_csize);
        r3dOutToLog("[%04d/%04d] file: %s ratio:%.2fx, size:%.0f mb\n",
            i, data->endIdx, fe.name, cperc, (float)(*data->stat_csize) / 1024 / 1024);

        OutputDebugStringA(fe.name);

        int cmethod = r3dFSCompress::COMPRESS_INFLATE; //ถ้าใช้ ตัว COMPRESS ของ ZSTD ให้แก้เป็น ชอง ZSTD
        ...
        EnterCriticalSection(&g_FileSysCritSection);
        *data->stat_size += fsize;
        *data->stat_csize += csize;

        int volume;
        DWORD offset;
        data->fileSystem->AllocSpaceForFile(csize, &volume, &offset);
        ...
        if (!fe.IsValid()) {
            LeaveCriticalSection(&g_FileSysCritSection);
            r3dError("!fe.IsValid()");
        }

        data->fileSystem->WriteFileData(fe, cdata, csize);
        LeaveCriticalSection(&g_FileSysCritSection);
...
bool r3dFileSystem::BuildNewArchive(const char* baseName) {
    ...
    if (numFiles >= 2000) 
	{
        ...
        int chunkSize = (numFiles + numThreads - 1) / numThreads;

        std::vector<HANDLE> threads;

        for (int i = 0; i < numThreads; ++i) {
            int startIdx = i * chunkSize;
            int endIdx = Min(startIdx + chunkSize, numFiles);
            ...
            data->endIdx = endIdx;
            data->stat_size = &stat_size;
            data->stat_csize = &stat_csize;

            HANDLE thread = CreateThread(
                NULL, 0, CompressFileInThread, data, 0, NULL
            );
	...
	else 
	{
        ...
        data->fileSystem = this;
        data->startIdx = 0;
        data->endIdx = numFiles;
        data->stat_size = &stat_size;
        data->stat_csize = &stat_csize;

        CompressFileInThread(data);
    }
