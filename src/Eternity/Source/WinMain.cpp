...
#ifdef FINAL_BUILD
const char * g_szApplicationName = "Warz Siam MMO By Gudgusgusxz";
#else
const char * g_szApplicationName = "Eclipse Studio";
#endif 

#ifndef WO_SERVER
int32_t	g_nProjectVersionMajor = 4;
int32_t	g_nProjectVersionMinor = 2;
int32_t g_nProjectVersionMinor2 = 1;
...
double getMemoryUsage()
{
	...
	double virtualMemUsed = (double)(memInfo.ullTotalPhys - memInfo.ullAvailPhys) / memInfo.ullTotalPageFile * 100;
	return virtualMemUsed;
}

std::string s = GetBuildVersionString();
unsigned __stdcall CPUMonitor(void *data)
{
	while (true) 
	{
		char textToWrite[512];
		sprintf_s(textToWrite, "%s | CPU : %2.0f%% | MEM : %2.0f%%", s.c_str(), getCurrentValue(), getMemoryUsage());
		SetWindowText(win::hWnd, textToWrite);
		Sleep(1000);
	}
	return 0;
}
...
void CPUInit()
{
	...
	_beginthreadex(NULL, 0, &CPUMonitor, NULL, 0, NULL);
}
#endif
