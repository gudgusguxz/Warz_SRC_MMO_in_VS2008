...
r3dInputMappingMngr::r3dInputMappingMngr()
{
	...
	m_Mapping[KS_INSPECT_GUN] =             KeyboardMapping(INPUTMAP_KEYBOARD, kbsB, "$HUD_Cntrl_InspectGun", false); //Show Gun
	m_Mapping[KS_SHOW_TAUNT] =              KeyboardMapping(INPUTMAP_KEYBOARD, kbsF, "$HUD_Cntrl_ShowTaunts", false); //Show Taunts
	m_Mapping[KS_TOGGLE_CHAT_HUD] =			KeyboardMapping(INPUTMAP_KEYBOARD, kbsF5, "$HUD_Toggle_Chat_HUD", false); //Show Chat
	m_Mapping[KS_SHOW_FRIENDS_NAME] =		KeyboardMapping(INPUTMAP_KEYBOARD, kbsF7, "$HUD_Cntrl_FriendsName", false);//AlexRedd:: For new friend name button
