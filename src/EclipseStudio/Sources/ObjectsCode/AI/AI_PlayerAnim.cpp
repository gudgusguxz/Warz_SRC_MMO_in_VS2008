...
void CUberData::LoadTauntAnim()
{
	...
	aid_.taunts[1] = AddAnimation("Taunt_t1_songkran2");//2+
	aid_.taunts[2] = AddAnimation("Taunt_t1_Litterbox");//8+
	aid_.taunts[3] = AddAnimation("Taunt_t1_Backflip");//4+
	aid_.taunts[4] = AddAnimation("Taunt_t1_HandFlip");//5
	aid_.taunts[5] = AddAnimation("Taunt_t1_PDead");//6+
	aid_.taunts[6] = AddAnimation("Taunt_t1_z");//7
	aid_.taunts[7] = AddAnimation("Taunt_t1_songkran1");//3+
	aid_.taunts[8] = AddAnimation("Taunt_t1_insult");//9
}
void CUberData::LoadPlayerInteractAnim()
{
	aid_.PlayerInteract[0] = AddAnimation("PickUpItem");
	aid_.PlayerInteract[1] = AddAnimation("PlayerTalkInteract");
}
////////////////////////////////////////////////////////////end
...
void CUberData::LoadAngleAnim()
{
	aid_.angles[0] = AddAnimation("Fishing_Idle");
	aid_.angles[1] = AddAnimation("Fishing_Shot");
	aid_.angles[2] = AddAnimation("Fishing_Collect");
}
...
void CUberEquip::DrawSlot(ESlot slotId, const D3DXMATRIX& world, DrawType dt, bool skin, bool draw_firstperson, const r3dSkeleton* wpnSkeleton)
{
	...
	if(draw_firstperson)
	{
		if(slotId == SLOT_Armor || slotId == SLOT_LowerBody || slotId == SLOT_Head || slotId == SLOT_Hair || slotId == SLOT_Helmet || slotId == SLOT_Backpack)
			return;
	}
...
CUberAnim::CUberAnim(obj_Player* in_player, CUberData* in_data)
{
	...
	DamageAnimTrack = INVALID_TRACK_ID; // GUN CONTENT
...
void CUberAnim::SwitchToState(int PlayerState, int MoveDir, int AnglingStatus)
{
	...
	switch(AnglingStatus)
	{
		case Angling_Idle:
			a1 = aid.angles[0];
			a2 = IsFPSMode() ? aid.angles[0] : -1;
			break;
		case Angling_Aim:
			a1 = aid.angles[1];
			a2 = IsFPSMode() ? aid.angles[1] : -1;
			break;
		case Angling_Store:
			a1 = aid.angles[2];
			a2 = IsFPSMode() ? aid.angles[2] : -1;
			break;
	}
...
void CUberAnim::SyncAnimation(int PlayerState, int MoveDir, bool force, const Weapon* weap, bool isInAttmMenu, int AnglingStatus, bool isWantingToInspect)
{
	if(weap)
	{
...
// 		// if throwing grenades from idle, switch to idleaim
	...
	else
	{
		if(IsInUI && (CurrentWeapon != weap || (weap == NULL && anim.AnimTracks.size() == 0)))
		{
			...
			if(AnglingStatus == Angling_Idle)
				anim.StartAnimation(data_->aid_.angles[0], ANIMFLAG_RemoveOtherFade | ANIMFLAG_Looped, 1.0f, 1.0f, 0.0f);
			else if(AnglingStatus == Angling_Aim)
				anim.StartAnimation(data_->aid_.angles[1], ANIMFLAG_RemoveOtherFade | ANIMFLAG_Looped, 1.0f, 1.0f, 0.0f);
			else if(AnglingStatus == Angling_Store)
				anim.StartAnimation(data_->aid_.angles[2], ANIMFLAG_RemoveOtherFade | ANIMFLAG_Looped, 1.0f, 1.0f, 0.0f);
			return;
		}
	...
	SwitchToState(PlayerState, MoveDir, AnglingStatus);
...
void CUberAnim::StartinsultAnim()
{
	StopReloadAnim();
	//StopGrenadeAnimations();
...
// GUN CONTENT
void CUberAnim::StartDamageAnim()
{
	StopReloadAnim();
	
	DamageAnimTrack = anim.StartAnimation(data_->aid_.taunts[random(7)] /*data_->aid_.taunts[6]*/ /*TAUNT INDEX*/, ANIMFLAG_PauseOnEnd | ANIMFLAG_RemoveOtherFade, 0.0f, 1.0f, 0.1f);
}
void CUberAnim::StopDamageAnim()
{
	if(DamageAnimTrack != INVALID_TRACK_ID)
	{
		anim.Stop(DamageAnimTrack);
		DamageAnimTrack = INVALID_TRACK_ID;
	}
}
//end
