...
extern const uint32_t	g_SuperZombieItemID2	= 20227;
...
struct SZombieRenderable : Renderable//AlexRedd:: boss health
{
	void Init()
	{
		DrawFunc = Draw;
	...
	static void Draw( Renderable* RThis, const r3dCamera& Cam )
	{
		SZombieRenderable* This = static_cast<SZombieRenderable*>( RThis );

		This->Parent->DrawSZInfo();
	}

	obj_Zombie* Parent;
};

void obj_Zombie::DrawSZInfo() const//AlexRedd:: boss health
{
	...
	if(gClientLogic().localPlayer_)
	{
		...
		{
			r3dPoint3D scrCoord;			
			if(r3dProjectToScreen(GetPosition() + r3dPoint3D(0, 2.5f, 0), &scrCoord))
			{				
				if(!bDead && m_bossHealth > 0.0f  && m_bossHealth < CreateParams.SZombieHealth && (hudPause && !hudPause->isActive()))
				{
					float maxHealth = CreateParams.SZombieHealth;					
					static float maxBarSize = 125.f;

					r3dColor24	clr;
					if (m_bossHealth < maxHealth/4)
						clr = r3dColor::red;
					else if (m_bossHealth < maxHealth/2)
						clr = r3dColor::orange;
					else
						clr = r3dColor::green;					

					r3dDrawBox2D( scrCoord.x - maxBarSize/2 - 1, scrCoord.y + 0, maxBarSize + 2, 11, r3dColor24::black );
					r3dDrawBox2D( scrCoord.x - maxBarSize/2 - 0, scrCoord.y + 1, maxBarSize + 0,  9, r3dColor24::grey );
					r3dDrawBox2D( scrCoord.x - maxBarSize/2 - 0, scrCoord.y + 1, m_bossHealth*maxBarSize/maxHealth, 9, clr );
...
obj_Zombie::obj_Zombie() 
	...
	, bDead( false )
	, m_bossHealth( 0.0f )//AlexRedd:: boss health
#ifndef FINAL_BUILD
	, m_minSpeed( 1000.0f )
	, m_maxSpeed( 0.0f )
...
BOOL obj_Zombie::OnCreate()
{
	...
	m_typeIndex = (CreateParams.HeroItemID == g_SuperZombieItemID) ? 1 : (CreateParams.HeroItemID == g_SuperZombieItemID2) ? 1 : 0;
	...
	ZombieAnimGroup	= (!IsSuperZombie() && CreateParams.FastZombie) ? ( 1 + u_random(2)) : (!IsSuperZombie2() && CreateParams.FastZombie) ? ( 1 + u_random(2)) : 0;
	...
	if (IsSuperZombie())
	{
		...
		m_bossHealth = CreateParams.SZombieHealth;//AlexRedd:: boss health
	}
	else
	{
...
{
	if(gClientLogic().localPlayer_ == NULL || bDead || !isSoundAudible() || !IsSuperZombie() || !IsSuperZombie2())
		return;
...
void obj_Zombie::UpdateConcurrent( int fullAnimUpdate )
{
	...
	if( g_enable_zombie_sprint->GetBool() ) // Turn on/off Sprinting
	{
			...
			( 0.0f < SprintTimer || 0.0f < SprintFalloffTimer ) )
		{
			...
			if(IsSuperZombie() || IsSuperZombie2() || !staggeredTrack)
			{
	...
	if( ( IsSuperZombie() || IsSuperZombie2() || !staggeredTrack ) && ( EZombieStates::ZState_Walk == ZombieAnimState || EZombieStates::ZState_Pursue == ZombieAnimState || EZombieStates::ZState_PursueSprint == ZombieAnimState ) )
	{
	...
	else if ( ( IsSuperZombie() || IsSuperZombie2() || !staggeredTrack ) && EZombieStates::ZState_Attack == ZombieAnimState )
	{
		if( IsSuperZombie() && IsSuperZombie2() && attackTrack )
		{
...
int obj_Zombie::StartIdleEatCrouchedAnim()
{
	...
	{
	default:
	case 0: aid = AddAnimation("Zombie_Eat_Crouched_04"); break;
	case 1: aid = AddAnimation("Zombie_Eat_Stand_1"); break;
	case 2: aid = AddAnimation("Zombie_Eat_Stand_2"); break;
	}
...
int obj_Zombie::PlayDeadStandAnim()
{
	...
	else
	{
		...
		case 1:	aid = AddAnimation("Zombie_Dead_Stand_B_02"); break;
		case 2:	aid = AddAnimation("Zombie_Dead_Stand_B_03"); break;
		case 3:	aid = AddAnimation("Zombie_Dead_Stand_F_01"); break;
		case 4:	aid = AddAnimation("Zombie_Dead_Stand_F_02"); break;
		case 5:	aid = AddAnimation("Zombie_Dead_Stand_F_03"); break;
		}
	}
...
void obj_Zombie::OnNetPacket(const PKT_S2C_SZombieHealth_s& n)
{
	if (IsSuperZombie())
	{
		m_bossHealth = n.SZombieHealth;
	}
}
...
#define DEFINE_GAMEOBJ_PACKET_HANDLER(xxx) \
	...
	switch(EventID)
	{
		...
		DEFINE_GAMEOBJ_PACKET_HANDLER(PKT_S2C_SZombieHealth);
...
{
	if (vehicle->GetSpeed() < 5.0f || ZombieState == EZombieStates::ZState_FakeDead || IsSuperZombie() || IsSuperZombie2())
		return;
