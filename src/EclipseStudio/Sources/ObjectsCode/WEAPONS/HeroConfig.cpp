...
r3dMesh* HeroConfig::getBodyMesh(int index, bool isFPS, const char* GearFPSModelName ) const
{
	...
	else
	{
		...
		if( GearFPSModelName != "" )
		{
			m_GearFPSMeshes = r3dGOBAddMesh(GearFPSModelName, true, false, true, true );

			if(m_GearFPSMeshes == NULL)
			{
				r3dError("ART: failed to load mesh '%s'\n", GearFPSModelName);
			}

			return m_GearFPSMeshes;
		}
...
bool HeroConfig::loadBaseFromXml(pugi::xml_node& xmlHero)
{
	...
	m_CharHair = xmlHero.child("HeroDesc").attribute("maxHairs").as_int() > 0? true:false;
    m_FemaleHero = xmlHero.child("HeroDesc").attribute("FemaleHero").as_int() > 0? true:false;
