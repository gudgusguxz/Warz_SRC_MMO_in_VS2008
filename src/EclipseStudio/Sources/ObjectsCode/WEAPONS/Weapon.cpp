...
void Weapon::Fire(const r3dPoint3D& hitPos, const D3DXMATRIX& weaponBone, bool executeWeaponFireCode, float cookingTime, const r3dPoint3D& grenadeFireFrom, GameObject** out_bulletObj)
{
	if(isUsableItem())
	{
		...
		if(m_Owner->NetworkLocal && m_pConfig->m_isConsumable && m_pConfig->m_itemID != WeaponConfig::ITEMID_FishingRod && m_pConfig->m_itemID != WeaponConfig::ITEMID_FishingRod1)
		{
			wiInventoryItem& wi = getPlayerItem();
			r3d_assert(wi.quantity > 0);
			wi.quantity--;
...
void Weapon::AdjustFishingWeaponDurability()
{
	if (m_BackpackIdx == HANDS_WEAPON_IDX)
		return;

	getPlayerItem().adjustFishingDurability(-getConfig()->m_DurabilityUse);
	...
	for (int atm = 0; atm<WPN_ATTM_MAX; atm++)
	{
		...
		for (int i = 0; i<m_Owner->CurLoadout.BackpackSize; i++)
		{
			...
			if (itm.itemID == wac->m_itemID)
			{
				...
				itm.adjustFishingDurability(-wac->m_DurabilityUse);
				break;
			}
		}
	}
...
void Weapon::StartReloadSequence()
{
	...
	m_lastReloadingTime = r3dGetTime();
	if(m_Owner->CurLoadout.Skills[CUserSkills::SKILL_WeaponDegradation3])//AlexRedd:: new skill
		m_ReloadTime = m_pConfig->m_reloadTime / 1.5f;
	else
