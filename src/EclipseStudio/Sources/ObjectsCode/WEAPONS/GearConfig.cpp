...
bool GearConfig::loadBaseFromXml(pugi::xml_node& xmlGear)
{
	...
	HideBody = xmlGear.child("Armor").attribute("HideBody").as_int() > 0 ? true:false;
    HideLegs = xmlGear.child("Armor").attribute("HideLegs").as_int() > 0 ? true:false;
	HideHair = xmlGear.child("Armor").attribute("HideHair").as_int() > 0 ? true:false;
	HideHead = xmlGear.child("Armor").attribute("HideHead").as_int() > 0 ? true:false;
	HasBodyFPS = xmlGear.child("Armor").attribute("HasBodyFPS").as_int() > 0 ? true:false;
