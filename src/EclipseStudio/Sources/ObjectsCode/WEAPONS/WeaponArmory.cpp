...
WeaponArmory::WeaponArmory()
{
	...
	m_NumExchangeLoaded = 0;
	m_NumBoxReadyPlayLoaded = 0;
...
WeaponArmory::~WeaponArmory()
{
	...
	r3d_assert(m_NumExchangeLoaded==0);
	r3d_assert(m_NumCraftingNewLoaded==0);
	...
	r3d_assert(m_NumBoxReadyPlayLoaded == 0);
}
...
bool WeaponArmory::Init()
{
	...
	{ 
		pugi::xml_node xmlItems = xmlDB.child("CraftingNew");
		pugi::xml_node xmlItem = xmlItems.child("Record");
		while(!xmlItem.empty())
		{
			loadCraftingNew(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	{ 
		pugi::xml_node xmlItems = xmlDB.child("Exchange");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while(!xmlItem.empty())
		{
			loadExchanges(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
	{
		pugi::xml_node xmlItems = xmlDB.child("BoxReady");
		pugi::xml_node xmlItem = xmlItems.child("Item");
		while (!xmlItem.empty())
		{
			loadBoxReadyPlay(xmlItem);
			xmlItem = xmlItem.next_sibling();
		}
	}
...
const ExchangeConfig* WeaponArmory::getExchangeConfig(uint32_t itemID) //Exchange System
{
	BaseItemConfig* item = NULL;
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_Exchange)
			return (const ExchangeConfig*)item;
	}
	return NULL;
}
...
const CraftingNewConfig* WeaponArmory::getCraftingNewConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	if(m_itemsHash.GetObject(itemID, &item))
	{
		if(item->category == storecat_CraftingNew)
			return (const CraftingNewConfig*)item;
	}
	return NULL;
}

const BoxReadyPlayConfig* WeaponArmory::getBoxReadyPlayConfig(uint32_t itemID)
{
	BaseItemConfig* item = NULL;
	if (m_itemsHash.GetObject(itemID, &item))
	{
		if (item->category == storecat_BoxReady)
			return (const BoxReadyPlayConfig*)item;
	}
	return NULL;
}
...
CraftingNewConfig* WeaponArmory::loadCraftingNew(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	uint32_t itemID = xmlItem.attribute("ID").as_uint();
	{
		...
		if(m_itemsHash.GetObject(itemID, &temp))
		{
			r3dArtBug("Trying to load recordids of Crafting_System with id '%d' that is already loaded!", itemID);
			return NULL;
		}
	}

	CraftingNewConfig* CraftingNew = game_new CraftingNewConfig(itemID);
	CraftingNew->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, CraftingNew);
	m_NumCraftingNewLoaded++;

	return CraftingNew;
}

ExchangeConfig* WeaponArmory::loadExchanges(pugi::xml_node& xmlItem)//Exchange system
{
	r3d_assert(!xmlItem.empty());
	...
	uint32_t itemID = xmlItem.attribute("itemID").as_uint();
	{
		...
		if(m_itemsHash.GetObject(itemID, &temp))
		{
			r3dArtBug("Trying to load recordids of Exchanges_System with id '%d' that is already loaded!", itemID);
			return NULL;
		}
	}

	ExchangeConfig* exchange = game_new ExchangeConfig(itemID);
	exchange->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, exchange);
	m_NumExchangeLoaded++;

	return exchange;
}

BoxReadyPlayConfig* WeaponArmory::loadBoxReadyPlay(pugi::xml_node& xmlItem)
{
	r3d_assert(!xmlItem.empty());
	...
	uint32_t itemID = xmlItem.attribute("itemID").as_uint();
	{
		...
		if (m_itemsHash.GetObject(itemID, &temp))
		{
			r3dArtBug("Trying to load recordids of BoxReadyPlay_System with id '%d' that is already loaded!", itemID);
			return NULL;
		}
	}

	BoxReadyPlayConfig* BoxReady = game_new BoxReadyPlayConfig(itemID);
	BoxReady->loadBaseFromXml(xmlItem);

	m_itemsHash.Add(itemID, BoxReady);
	m_NumBoxReadyPlayLoaded++;
