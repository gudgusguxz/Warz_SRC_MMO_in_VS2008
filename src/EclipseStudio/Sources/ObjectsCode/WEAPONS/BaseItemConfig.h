...
class ExchangeConfig : public ModelItemConfig
{
public:
	ExchangeConfig(uint32_t id) : ModelItemConfig(id), numComponents(0) {}

	virtual bool loadBaseFromXml(pugi::xml_node& xmlItem);

	static const int MAX_NUM_COMPONENTS=1000;
	struct ExchangeComponent
	{
		uint32_t NeededItemID;
		uint32_t NeededQuantity;
		ExchangeComponent() {};
		ExchangeComponent(uint32_t id, uint32_t q):NeededItemID(id), NeededQuantity(q) {}
	};
	ExchangeComponent components[MAX_NUM_COMPONENTS];
	uint32_t numComponents;

	uint32_t ExchangedItemID;
	uint32_t ExchangedQuantity;
};

class CraftingNewConfig : public ModelItemConfig
{
public:
	CraftingNewConfig(uint32_t id) : ModelItemConfig(id), numComponents(0) {}

	virtual bool loadBaseFromXml(pugi::xml_node& xmlItem);

	static const int MAX_NUM_COMPONENTS=1000;
	struct CraftingNewComponent
	{
		uint32_t ComponentCraftingItemID;
		uint32_t ComponentCraftingQuantity;
		bool Return;
		
		CraftingNewComponent() {};
		CraftingNewComponent(uint32_t id, uint32_t q, bool r):ComponentCraftingItemID(id), ComponentCraftingQuantity(q), Return(r) {}
	};
	CraftingNewComponent components[MAX_NUM_COMPONENTS];
	uint32_t numComponents;

	uint32_t CraftingItemID;
	uint32_t CraftingQuantity;
	float Chance;
	int CraftedPage;
};

class BoxReadyPlayConfig : public ModelItemConfig
{
public:
	BoxReadyPlayConfig(uint32_t id) : ModelItemConfig(id), numComponents(0) {}

	virtual bool loadBaseFromXml(pugi::xml_node& xmlItem);

	static const int MAX_NUM_COMPONENTS = 1000;
	struct BoxReadyComponent
	{
		uint32_t ItemID;
		uint32_t Quantity;

		BoxReadyComponent() {};
		BoxReadyComponent(uint32_t id, uint32_t q) :ItemID(id), Quantity(q){}
	};
	BoxReadyComponent components[MAX_NUM_COMPONENTS];
	uint32_t numComponents;
};