...
bool BaseItemConfig::loadBaseFromXml(pugi::xml_node& xmlItem)
{
	...
	m_RdbLoot = xmlItem.child("LootID").attribute("id").as_int();
	...
	m_SellItem = xmlItem.child("SellItem").attribute("Price").as_int();
	m_SellGCItem = xmlItem.child("SellItem").attribute("PriceGC").as_int();
...
bool BoxReadyPlayConfig::loadBaseFromXml(pugi::xml_node& xmlItem)
{
	ModelItemConfig::loadBaseFromXml(xmlItem);
	if (m_itemID)
	{
		pugi::xml_node complist = xmlItem.child("Component");
		while (complist)
		{
			uint32_t ItemID = complist.attribute("ItemID").as_uint();
			uint32_t Quantity = complist.attribute("qty").as_uint();

			components[numComponents++] = BoxReadyComponent(ItemID, Quantity);
			complist = complist.next_sibling();
		}
	}
	else
	{
		r3d_assert(false && "Unsupported BoxReadyPlay item!!!");
	}
	return true;
}
...
bool CraftingNewConfig::loadBaseFromXml(pugi::xml_node& xmlItem)
{
	ModelItemConfig::loadBaseFromXml(xmlItem);
	if(m_itemID) 
	{
		...
		{
			uint32_t ItemID = complist.attribute("ItemID").as_uint();
			uint32_t Quantity = complist.attribute("Quantity").as_uint();
			bool Return = complist.attribute("Return").as_bool();

			components[numComponents++] = CraftingNewComponent(ItemID, Quantity, Return);
			complist = complist.next_sibling();
		}
		CraftingItemID = xmlItem.child("Crafting").attribute("ItemID").as_uint();
		CraftingQuantity = xmlItem.child("Crafting").attribute("qty").as_uint();
		Chance = xmlItem.child("Crafting").attribute("Chance").as_float();
		CraftedPage = xmlItem.child("Crafting").attribute("Page").as_int();
	}
	else
	{
		r3d_assert(false && "Unsupported Crafting item!!!");
	}
	return true;
}

bool ExchangeConfig::loadBaseFromXml(pugi::xml_node& xmlItem)
{
	ModelItemConfig::loadBaseFromXml(xmlItem);
	if(m_itemID) {
		if(xmlItem.child("NeededItemID").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID").attribute("ID").as_uint(), xmlItem.child("NeededQuantity").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID2").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID2").attribute("ID").as_uint(), xmlItem.child("NeededQuantity2").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID3").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID3").attribute("ID").as_uint(), xmlItem.child("NeededQuantity3").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID4").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID4").attribute("ID").as_uint(), xmlItem.child("NeededQuantity4").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID5").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID5").attribute("ID").as_uint(), xmlItem.child("NeededQuantity5").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID6").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID6").attribute("ID").as_uint(), xmlItem.child("NeededQuantity6").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID7").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID7").attribute("ID").as_uint(), xmlItem.child("NeededQuantity7").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID8").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID8").attribute("ID").as_uint(), xmlItem.child("NeededQuantity8").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID9").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID9").attribute("ID").as_uint(), xmlItem.child("NeededQuantity9").attribute("ID").as_uint());

		if(xmlItem.child("NeededItemID10").attribute("ID").as_uint() > 0)
			components[numComponents++] = ExchangeComponent(xmlItem.child("NeededItemID10").attribute("ID").as_uint(), xmlItem.child("NeededQuantity10").attribute("ID").as_uint());

		ExchangedItemID = xmlItem.child("ExchangedItemID").attribute("ID").as_uint();
		ExchangedQuantity = xmlItem.child("ExchangedQuantity").attribute("ID").as_uint();
	} else {
		r3d_assert(false && "Unsupported exchange item!!!");
	}
	return true;
}
