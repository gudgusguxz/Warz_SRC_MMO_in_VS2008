...
public:
	...
	const ExchangeConfig* getExchangeConfig(uint32_t itemID);
	const CraftingNewConfig* getCraftingNewConfig(uint32_t itemID);
	const BoxReadyPlayConfig* getBoxReadyPlayConfig(uint32_t itemID);
	...
	int getNumExchanges() const { return m_NumExchangeLoaded; }
	int getNumCraftingNew() const { return m_NumCraftingNewLoaded; }
...
protected:
	...
	virtual ExchangeConfig* loadExchanges(pugi::xml_node& xmlItem);
	virtual CraftingNewConfig* loadCraftingNew(pugi::xml_node& xmlItem);
	virtual BoxReadyPlayConfig* loadBoxReadyPlay(pugi::xml_node& xmlItem);
	...
	uint32_t	m_NumExchangeLoaded;
	uint32_t	m_NumCraftingNewLoaded;
	uint32_t	m_NumBoxReadyPlayLoaded;
