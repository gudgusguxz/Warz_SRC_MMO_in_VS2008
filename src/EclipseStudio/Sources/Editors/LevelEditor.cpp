...
int LoadLevel( float startLoadingProgress )
{
	...
	///////////////////////////////////////////////////////////////////////////////////// OLD
	{
		//AlexRedd:: For new render grass opt.
		//AlexRedd: BUG FIX: WE SHOULD ALWAYS LOAD GRASS ON STUDIO!
#ifdef FINAL_BUILD
		if (r_disable_grass->GetBool())
		{
			LoadGrassLib();
			//r3dOutToLog( "g_pGrassMap->Load...\n" );
			g_pGrassMap->Load(r3dGameLevel::GetHomeDir());
		...
		else if (r_disable_grass->GetBool() && !g_bEditMode)
		{
			LoadGrassLib();
			g_pGrassMap->Load(r3dGameLevel::GetHomeDir());
#ifndef FINAL_BUILD
	...
	case SE_OPTIONS_MENU:
		{
			...
			const int SSAO_LIST_HEIGHT = 132;

			stringlist_t ssaoNames;
			ssaoNames.push_back( "VERY LOW" );
...
float Editor_Level :: ProcessTerrain3_Settings( float SliderX, float SliderY )
{
	...
	if( allowTerrain3 )
	{
		...
		// quality settings
		{
			...
			qualityList.push_back( "Very Low" ) ;
...
void DebugPlayerDie()
{
	if( g_bEditMode )
	{
		for(GameObject *obj = GameWorld().GetFirstObject(); obj; obj = GameWorld().GetNextObject(obj) )
		{
			if( obj->isObjType(OBJTYPE_Human) )
			{
				static_cast< obj_Player* >( obj )->DoDeath( 0, true, storecat_INVALID, 0, 0 );
			}
		}
	}
}
