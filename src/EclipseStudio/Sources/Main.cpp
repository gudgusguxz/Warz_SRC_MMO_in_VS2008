...
void applyGraphicOptionsSoft( uint32_t settingsFlags )
{
	...
	if( settingsFlags & FrontEndShared::SC_TEXTURE_QUALITY )
	{
		...
		{
		case 1:
			r_sector_keep_alive_coef->SetFloat( 0.1f );
			break;
		case 2:
...
//	InitPointLightsRendererV2();
	...
	case	Menu_AppSelect::bUpdateDB:
		...
		MessageBox(0, "Disabled now!", "Result", MB_OK);
		break;
#endif

	case Menu_AppSelect::bStartGamePublic:
		// override server settings if special key isn't set
		if(strstr(__r3dCmdLine, "-ffgrtvzdfsdk") == NULL)//-ffgrtvzdf
		{
			// hardcoded IP for now
			g_serverip->SetString("26.224.36.102");
		}
		// override API settings
		g_api_ip->SetString("26.224.36.102");
...
void UpdateDB(const char* api_addr, const char* out_xml)
{
	...
	// get items DB
	{
		CkHttpRequest req;
		req.UsePost();
		req.put_Path("/SV-SIAM/api/php/api_getItemsDB.php");
		req.AddParam("serverkey", "bcKixGe84z5vrvQ");
