...
int CUserProfile::GetProfile(int CharID)
{
	...
	ActivePinAccount			  = xmlAccount.attribute("PinAcc").as_int();
	BadPinLoginCount			  = xmlAccount.attribute("BadPin").as_int();
	PinLoginStatus				  = xmlAccount.attribute("PinStatus").as_int();
...
static void parseCharBackpack(pugi::xml_node xmlItem, wiCharDataFull& w)
{
	...
	while(!xmlItem.empty())
	{
		if(xmlItem.attribute("qt").as_uint() <= 0)
		{
...
void CUserProfile::ParseLoadouts(pugi::xml_node& xmlItem)
{
	...
	while(!xmlItem.empty())
	{
		...
		w.GamertagColor   =	xmlItem.attribute("GamertagColor").as_int();
...
void CUserProfile::DeriveGamePricesFromItems()
{
	for(uint32_t i = 0; i<g_NumStoreItems; i++) 
	{
		...
		switch(itm.itemID) 
		{
			...
			case 301257: ShopPremiumAcc7 = itm.pricePerm; break;
		    case 301256: ShopPremiumAcc15 = itm.pricePerm; break;
			case 301259: ShopPremiumAcc30 = itm.pricePerm; break;
...
int CClientUserProfile::ApiBuyItem(int itemId, int buyIdx, __int64* out_InventoryID, int qty) //BuyQuantity
{
	r3d_assert(buyIdx > 0);
	...
	req.AddParam("Qty", qty); //BuyQuantity
	if(!req.Issue())
	{
...
int CClientUserProfile::ApiSetPinAccount(const char* code)
{
	r3d_assert(code);
	CWOBackendReq req(this, "api_AccApplyKey.aspx");
	req.AddParam("func", "pin");
	req.AddParam("serial", code);

	if (!req.Issue())
	{
		r3dOutToLog("ApiSetPinAccount FAILED, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
...
int CClientUserProfile::ApiRequestPin1stReset()
{
	CWOBackendReq req(this, "api_AccApplyKey.aspx");
	req.AddParam("func", "res");

	if (!req.Issue())
	{
		r3dOutToLog("ApiRequestPin1stReset FAILED, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
...
int CClientUserProfile::ApiUnlockPinAccount(const char* code)
{
	r3d_assert(code);
	CWOBackendReq req(this, "api_AccApplyKey.aspx");
	req.AddParam("func", "unp");
	req.AddParam("serial", code);

	if (!req.Issue())
	{
		r3dOutToLog("ApiUnlockPinAccount FAILED, code: %d\n", req.resultCode_);
		return req.resultCode_;
	}
