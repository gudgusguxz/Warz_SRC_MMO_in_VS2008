...
enum STORE_CATEGORIES
{
	...
	storecat_Exchange	= 57,
	storecat_CraftingNew	= 58,
	storecat_BoxReady = 59,
  ...
  public:
	...
	int adjustDurability(float perc)
	{
		if(perc != 0.0f)
		{
			...
			Var3 = 10000; //R3D_CLAMP(Var3, 0, (int)MAX_DURABILITY);
		}
		return Var3;
	}

	int adjustFishingDurability(float perc)
	{
		if (perc != 0.0f)
		{
			if (Var3 < 0) Var3 = MAX_DURABILITY;
...
// NOTE: you MUST increase P2PNET_VERSION if you change this structure
	...
	enum
	{
	  ...
	  GAMEFLAG_RadioactiveArea = (1 << 1),
	  GAMEFLAG_RadioactiveAreaNear = (1 << 2),
	  GAMEFLAG_NearFishing = (1 << 3),
	};
	...
	wiInventoryItem* hasItemQty(uint32_t itemID)
	{
		for (int i = 0; i<BackpackSize; ++i)
			if (Items[i].itemID == itemID && Items[i].quantity > 0)
				return &Items[i];
...
class CUserProfile
{
  public:
	...
	DWORD		SessionID;
	int		AccountStatus;
	int		ActivePinAccount;
	int		BadPinLoginCount;
	int		PinLoginStatus;
	...
	int		ShopCharRevive;
	int		ShopPremiumAcc7;
	int		ShopPremiumAcc15;
	int		ShopPremiumAcc30;
...
class CClientUserProfile : public CUserProfile
{
  ...
  public:
	...
	int		ApiGetItemsInfo();
	int		ApiBuyItem(int itemId, int buyIdx, __int64* out_InventoryID, int qty); //BuyQuantity
	...
	*/
	...
	int		ApiSetPinAccount(const char* code);
	int		ApiUnlockPinAccount(const char* code);
	int		ApiRequestPin1stReset();
