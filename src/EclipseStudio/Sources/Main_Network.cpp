...
extern CBandiCaptureLibrary m_bandiCaptureLibrary;
...
void ExecuteNetworkGame()
{
	...
	int mainmenuTheme = -1;
	if (r_enable_alpha_sounds->GetBool())
		mainmenuTheme = SoundSys.GetEventIDByPath("Sounds/MainMenu GUI/UI_MENU_MUSIC_ALFA");
	else
...
static EGameResult PlayNetworkGame()
{
	...
	extern void SMToggleRecordVideo();
	if ( m_bandiCaptureLibrary.IsCapturing() ) SMToggleRecordVideo();
