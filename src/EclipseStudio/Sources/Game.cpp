...
#include "bandicap.h"
#include "ui/HUDDisplay.h" // extern hudmain function
...
void SMToggleRecordVideo()
{
	r3dOutToLog("Toggle Bandicam Record!");
	IDirect3DDevice9 *pd3d9Device = r3dRenderer->pd3ddev;
	
	if (m_bandiCaptureLibrary.IsCapturing() == FALSE) // Not Recording -> Start Recording
	{
		if (m_bandiCaptureLibrary.IsCreated() == FALSE)
		{
			if (FAILED(m_bandiCaptureLibrary.Create(BANDICAP_RELEASE_DLL_FILE_NAME)))
				ASSERT(0);
			if (FAILED(m_bandiCaptureLibrary.Verify("MAIET-GUNZ-20080916", "d25f910a"))) // Trial Serial => DO NOT CHANGE!
				ASSERT(0);
		}
		if (m_bandiCaptureLibrary.IsCreated())
		{
			BCAP_CONFIG cfg;
			BCapConfigPreset(&cfg, BCAP_PRESET_MPEG4);
			m_bandiCaptureLibrary.CheckConfig(&cfg);
			m_bandiCaptureLibrary.SetConfig(&cfg);

			m_bandiCaptureLibrary.SetMinMaxFPS(80, 999);

			TCHAR pathName[MAX_PATH]; mkdir("Record");
			m_bandiCaptureLibrary.MakePathnameByDate(_T("Record\\"), _T("Siam"), _T("mp4"), pathName, MAX_PATH); // Using timestamp with Infest prefix

			HRESULT hr = m_bandiCaptureLibrary.Start(pathName, NULL, BCAP_MODE_D3D9_SCALE, (LONG_PTR)pd3d9Device);

			if (FAILED(hr)) // Failed with some reason
				ASSERT(0);

			if (hudMain) hudMain->addChatMessage(0, "<Camera>", "Recording...", 1);
		}
	}
	else // Already Recording -> Stop Recording
	{
		m_bandiCaptureLibrary.Stop();
		if (hudMain) hudMain->addChatMessage(0, "<Camera>", "Stopped Recording...", 1);
	}
}
...
void DoLoadGame(const char* LevelName, int MaxPlayers, bool unloadPrev, bool isMenuLevel )
{
	...
	if (!r_increase_fps->GetBool())//Increase FPS option..	
		gCam.SetPlanes(r_near_plane->GetFloat(), r_far_plane->GetFloat());
	else
		gCam.SetPlanes(r_near_plane->GetFloat(), 800.f);
...
void GameStateGameLoop()
{
	...
	if( Keyboard->WasPressed( kbsF9 ) ) //ปุ่มอัดคลิป
		SMToggleRecordVideo();
	...
	{
		IDirect3DDevice9 *pd3d9Device = r3dRenderer->pd3ddev;
		HRESULT hr = m_bandiCaptureLibrary.Work((LONG_PTR)pd3d9Device);
		if (FAILED(hr))
		{
			SMToggleRecordVideo();
			...
			{
				r3dOutToLog("Cannot create output file\n");
				if (hudMain)
					hudMain->addChatMessage(0, "<Camera>", "Error: Cannot create output file", 1);
			}
			else if (hr == BCERR_FILE_WRITE)
			{
				r3dOutToLog("Cannot write output file\n");
				if (hudMain)
					hudMain->addChatMessage(0, "<Camera>", "Error: Cannot write output file", 1);
			}
			else if (hr == BCERR_NOT_ENOUGH_DISKSPACE)
			{
				r3dOutToLog("Not enough free disk space\n");
				if (hudMain)
					hudMain->addChatMessage(0, "<Camera>", "Error: Not enough free disk space", 1);
			}
			else if (hr == BCERR_VIDEO_CAPTURE)
			{
				r3dOutToLog("Fail to capture video\n");
				if (hudMain)
					hudMain->addChatMessage(0, "<Camera>", "Error: Fail to capture video", 1);
			}
			else if (hr == BCERR_AUDIO_CAPTURE)
			{
				r3dOutToLog("Fail to capture audio\n");
				if (hudMain)
					hudMain->addChatMessage(0, "<Camera>", "Error: Fail to capture audio", 1);
			}
		}
	}
