...
DWORD SetDefaultSettings( r3dDevStrength strength )
{
	...
	r_apex_enabled->SetBool(r_overall_quality->GetInt() == 5);
...
{	
		...
		*damage = (int)(gc->m_damagePerc*100.0f);
...
void addItemsAndCategoryToUI(r3dScaleformMovie& gfxMovie)
{
	...
	r3dgameVector(const BoxReadyPlayConfig*) allBoxReady;
	...
	while(g_pWeaponArmory->searchNextItem())
	{
		...
		const BoxReadyPlayConfig* BoxReady = g_pWeaponArmory->getBoxReadyPlayConfig(itemID);
		if (BoxReady)
		{
			allBoxReady.push_back(BoxReady);
		}
	...
	const size_t ItemBoxReadySize = allBoxReady.size();
	for (size_t i = 0; i < ItemBoxReadySize; ++i)
	{
		const BoxReadyPlayConfig* BoxReadyConf = allBoxReady[i];

		var[0].SetUInt(BoxReadyConf->m_itemID);
		var[1].SetNumber(BoxReadyConf->category);
		var[2].SetString(BoxReadyConf->m_StoreName);
		var[3].SetString(BoxReadyConf->m_Description);
		var[4].SetString(BoxReadyConf->m_StoreIcon);
		var[5].SetBoolean(false);
		var[6].SetNumber(BoxReadyConf->m_Weight);
		gfxMovie.Invoke("_root.api.addItem", var, 7);
	}

	var[0].SetNumber(storecat_BoxReady);
	var[1].SetString("storecat_BoxReady");
	var[2].SetNumber(9);
	var[3].SetNumber(4);
	gfxMovie.Invoke("_root.api.addCategory", var, 4);
