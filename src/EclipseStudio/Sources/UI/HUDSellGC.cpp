...
void HUDSellGC::eventRepairAll(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...
void HUDSellGC::eventRepairItemInSlot(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
...
void HUDSellGC::eventReturnToGame(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	Deactivate();
}

void HUDSellGC::setErrorMsg(const char* errMsg)
{
	if (isLockedUI)
	{
		isLockedUI = false;
...
bool HUDSellGC::Init()
{
	if (!gfxMovie.Load("Data\\Menu\\WarZ_HUD_SellGC.swf", false))
		return false;

	char TopText[256] = { 0 };
	if (gUserProfile.ProfileData.PremiumAcc > 0)
		sprintf_s(TopText, sizeof(TopText), "", gLangMngr.getString("$FR_SaleTitle1"), gLangMngr.getString("$FR_SaleTitle2"));
	else
		sprintf_s(TopText, sizeof(TopText), "", gLangMngr.getString("$FR_SaleTitle1"), gLangMngr.getString("$FR_SaleTitle2"));
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDSellGC>(this, &HUDSellGC::FUNC)
...
void HUDSellGC::updateSurvivorTotalWeight()
{
	obj_Player* plr = gClientLogic().localPlayer_;
	r3d_assert(plr);
...
int ItemPriceGC(const wiInventoryItem &wi1)
{
	int tmpPrice = 0;
...
void HUDSellGC::reloadBackpackInfo()
{
	updateSurvivorTotalWeight();
	// reset backpack
	{
	...
	updateMoney(gUserProfile.ProfileData.GamePoints);
	gfxMovie.Invoke("_root.api.Main.setGD", gUserProfile.ProfileData.GamePoints);
	gfxMovie.Invoke("_root.api.Main.showBackpack", "");
}

void HUDSellGC::updateMoney(int gc)
{
	gfxMovie.Invoke("_root.api.setGD", gc);
}