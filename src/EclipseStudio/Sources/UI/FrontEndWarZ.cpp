...
DWORD NameTagHack_Addr= NULL;
BYTE NameTagHackMem[16];
...
const char* getReputationString(int reputation)
{
	...
	if(reputation >= 2000000)
		algnmt = "$rep_president";

	else if(reputation >= 800000)
		algnmt = "$rep_cia";

	else if(reputation >= 450000)
		algnmt = "$rep_fbi";
...
//
	else if(reputation <= -2400000)
		algnmt = "$rep_ferrari";

	else if(reputation <= -1000000)
		algnmt = "$rep_hell";

	else if(reputation <= -600000)
		algnmt = "$rep_monster";
...
{
	if(mapID == GBGameInfo::MAPID_WZ_Colorado) return gLangMngr.getString("Colorado");
	else if(mapID == GBGameInfo::MAPID_WZ_Cliffside) return gLangMngr.getString("Cliffside");
	else if(mapID == GBGameInfo::MAPID_WZ_California) return gLangMngr.getString("California");
	else if(mapID == GBGameInfo::MAPID_WZ_Caliwood) return gLangMngr.getString("Caliwood");
	else if(mapID == GBGameInfo::MAPID_WZ_AircraftCarrier) return gLangMngr.getString("Aircraft");
	else if(mapID == GBGameInfo::MAPID_WZ_Trade_Map) return "TRADEMAP";
	else if(mapID == GBGameInfo::MAPID_ServerTest) return "DEVMAP";

	    /////////////////////////////////////////

	else if(mapID == GBGameInfo::MAPID_PVP01) return "Clearview";
	else if(mapID == GBGameInfo::MAPID_PVP02) return "Rockyford";
	else if(mapID == GBGameInfo::MAPID_PVP03) return "Bigcity";
	else if(mapID == GBGameInfo::MAPID_PVP04) return "Sesamo";
	else if(mapID == GBGameInfo::MAPID_PVP05) return "Market";
	else if(mapID == GBGameInfo::MAPID_PVP06) return "MarketNodrop";
	else if(mapID == GBGameInfo::MAPID_PVP07) return "Container";
	else if(mapID == GBGameInfo::MAPID_PVP08) return "Snowvillage";
	else if(mapID == GBGameInfo::MAPID_PVP09) return "Ecosnow";
	else if(mapID == GBGameInfo::MAPID_PVP10) return "Paintball";
	else if(mapID == GBGameInfo::MAPID_PVP11) return "Bunker";
	else if(mapID == GBGameInfo::MAPID_PVP12) return "RockyfordV2";
	else if(mapID == GBGameInfo::MAPID_PVP13) return "Lego";
	else if(mapID == GBGameInfo::MAPID_PVP14) return "ClearviewOrg";
	else if(mapID == GBGameInfo::MAPID_PVP15) return "RockyfordOrg";

	else if(mapID == GBGameInfo::MAPID_PVP16) return "Frosty";
	else if(mapID == GBGameInfo::MAPID_PVP17) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP18) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP19) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP20) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP21) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP22) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP23) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP24) return "PVPSERVER";
	else if(mapID == GBGameInfo::MAPID_PVP25) return "PVPSERVER";
...
, r3dIResource(r3dIntegrityGuardian())
{
	...
	m_browseGamesMode = 0;
	if(NameTagHack_Addr == NULL)
	{
		__asm
		{
			mov ebx,Scaleform::Render::TreeContainer::NodeData::PropagateUp
			mov NameTagHack_Addr,ebx
			/*mov NameTagHackMem1,[ebx + 0xBA]
			mov NameTagHackMem2,[ebx + 0xBE]
			mov NameTagHackMem3,[ebx + 0xC2]*/
		}
		memcpy((void*)NameTagHackMem,(void*)(NameTagHack_Addr + 0xBA),16);
		//r3dOutToLog("(NameTagHack_Addr + 0xBA): %x\n", (NameTagHack_Addr + 0xBA));
		/*NameTagHackMem1 = *(DWORD*)(NameTagHack_Addr + 0xBA);
		NameTagHackMem2 = *(DWORD*)(NameTagHack_Addr + 0xBE);
		NameTagHackMem3 = *(DWORD*)(NameTagHack_Addr + 0xC2);
		NameTagHackMem3 = *(DWORD*)(NameTagHack_Addr + 0xC2);*/
	}
}
...
static bool ActualGetProfileData(FrontendWarZ* UI)
{
	...
	if (g_PinCodeFirstStart->GetBool())
	{
		SetLoadStage("ApiRequestPin");
		if (gUserProfile.ApiRequestPin1stReset() != 0)
			return false;
	}
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<FrontendWarZ>(this, &FrontendWarZ::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventRequestPinAccount", MAKE_CALLBACK(eventRequestPinAccount));
	gfxMovie.RegisterEventHandler("eventSetPinKeyAccount", MAKE_CALLBACK(eventSetPinKeyAccount));
	gfxMovie.RegisterEventHandler("eventUnlockPinKeyAccount", MAKE_CALLBACK(eventUnlockPinKeyAccount));
	...
	gfxMovie.RegisterEventHandler("eventExtraOptionsApply", MAKE_CALLBACK(eventExtraOptionsApply));
	...
	gfxMovie.RegisterEventHandler("eventRequestLotteryData", MAKE_CALLBACK(eventRequestLotteryData));
...
int FrontendWarZ::Update()
{
	...
	if(asyncThread_ == NULL)
	{
		...
		if(!gLoginSessionPoller.IsConnected()) {
			...
			*(DWORD*)0 = 0; //Just crash the game, not need to get error message..
		}
...
void FrontendWarZ::initFrontend()
{
	...
	if (!gUserProfile.ActivePinAccount)
	{
		gfxMovie.Invoke("_root.api.Main.OptionsGeneral.HidePin", true);
	}
	else
	{
		if (g_PinCodeFirstStart->GetBool())
			gfxMovie.Invoke("_root.api.showPinScreen", true);
	}
	...
	{
		var[0].SetBoolean(true/*gUserProfile.IsTrialAccount() || gUserProfile.ProfileData.isDevAccount>0*/); // trial channel
		...
		var[3].SetBoolean(gUserProfile.ShopPremiumAcc7>0 || gUserProfile.ShopPremiumAcc15>0  || gUserProfile.ShopPremiumAcc30>0 || gUserProfile.ProfileData.PremiumAcc>0); // premium servers (enable only if premium is available for sale or player alreayd has premium account
		var[4].SetBoolean(true/*!gUserProfile.IsTrialAccount()*/); // strongholds		
		var[5].SetBoolean(true/*gUserProfile.ShopPremiumAcc>0 || gUserProfile.ProfileData.PremiumAcc>0*/); //DEV EVENT SERVERS 
		var[6].SetBoolean(true/*!gUserProfile.IsTrialAccount()*/); // veteran servers
	...
	gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.BtnUpgtrial.visible", gUserProfile.ProfileData.AccountType == 2 ? true : false);
	gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.BtnUpgtrial.Text.Text.text", gLangMngr.getString("$FR_UpgradeTrialAccount"));
	if(gUserProfile.ProfileData.isDevAccount==0)
		gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.PremiumAcc.visible", gUserProfile.ProfileData.PremiumAcc>0 ? true : false);
	else
		gfxMovie.SetVariable("_root.api.Main.SurvivorsAnim.Survivors.DEVAcc.visible", gUserProfile.ProfileData.isDevAccount>0 ? true : false);
	...
	gfxMovie.SetVariable("_root.api.PremiumAccount_Price", gUserProfile.ShopPremiumAcc7);
	gfxMovie.SetVariable("_root.api.PremiumAccount15_Price", gUserProfile.ShopPremiumAcc15);
	gfxMovie.SetVariable("_root.api.PremiumAccount30_Price", gUserProfile.ShopPremiumAcc30);
	...
	{
		g_user_language->SetString("english"); // disable other language
		writeGameOptionsFile();
		ReloadLocalization();
	}
...
void FrontendWarZ::eventCreateCharacter(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(strpbrk(m_CreateGamerTag, "!@#$%^&*()-=+_<>,./?'\":;|{}[] °¢£§•¶ß®©™´¨≠ÆØ∞±≤≥¥µ∂∑∏π∫ªºΩ æø¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“ ”‘’÷◊ÿŸ⁄‡·‚„‰ÂÊÁËÈÍÎÏÌÓ")!=NULL) // do not allow this symbols
	{
		Scaleform::GFx::Value var[2];
...
void FrontendWarZ::eventBuyItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	//BuyQuantity
	mStore_BuyQuantity = args[3].GetInt();
	mStore_BuyPrice *= mStore_BuyQuantity;
	mStore_BuyPriceGD *= mStore_BuyQuantity;

		if(mStore_BuyQuantity < 1)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetString("Wrong Quantity");
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}

	if(mStore_BuyQuantity > 999)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetString("Max Buy is 999");
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}
...
bool FrontendWarZ::ParseGameJoinAnswer()
{
	...
	case GBPKT_M2C_JoinGameAns_s::rJoinDelayActive:
		...
		return false;
	case GBPKT_M2C_JoinGameAns_s::rFullPlayerClanLimit:
		SetAsyncError(0, gLangMngr.getString("FullPlayerClanLimit"));
		return false;
	}
...
unsigned int WINAPI FrontendWarZ::as_JoinGameThread(void* in_data)
{
	...
	{
		gMasterServerLogic.SendJoinGame(This_->m_joinGameServerId, This_->m_joinClanID, This_->m_joinGamePwd);
		if (This_->WaitForGameJoinAnswer() == false)
			break;
...
unsigned int WINAPI FrontendWarZ::as_BuyItemThread(void* in_data)
{
	...
	int apiCode = gUserProfile.ApiBuyItem(This->mStore_BuyItemID, buyIdx, &This->m_inventoryID, This->mStore_BuyQuantity); //BuyQuantity
	if(apiCode != 0)
	{
...
bool FrontendWarZ::UpdateInventoryWithBoughtItem()
{
	...
	int quantityToAdd = storecat_GetItemBuyStackSize(mStore_BuyItemID) * mStore_BuyQuantity; //BuyQuantity
	int totalQuantity = 1;
	...
	if(mStore_BuyItemID == 900100)//filter for lottery ticket ItemID
		return true;
...
void FrontendWarZ::SyncGraphicsUI()
{
	...
	{		//AlexRedd:: Extra options
		var[0].SetNumber( r_chat_message_sound->GetInt()+1);//Chat sound
		var[1].SetNumber( r_crosshaire_mode->GetInt()+1);//Crosshair mode
		var[2].SetNumber( r_highlight_mode->GetInt()+1);//Highlight mode
		var[3].SetNumber( r_increase_fps->GetInt()+1); //Increase FPS
		var[4].SetNumber( r_disable_grass->GetInt()+1); //disable grass
		var[5].SetNumber( r_disable_jump_sound->GetInt()+1); //Disable jump sound
		var[6].SetNumber( r_enable_alpha_sounds->GetInt()+1);// Alpha music, sounds
		var[7].SetNumber( r_enable_killfeed_messages->GetInt()+1);// KillFeed messages
		var[8].SetNumber( r_enable_killstreak_sounds->GetInt()+1);// KillSreak sounds enable
		var[9].SetNumber( r_killstreak_sounds->GetInt()+1);// KillSreak sounds pack (0-crossfire, 1-quake, Duke)
		var[10].SetNumber( r_crosshaire_color->GetInt() + 1);// Crosshaire color (0 ="white", 1 ="red", 2 ="blue", 3 ="green")
		gfxMovie.Invoke("_root.api.setExtraOptions", var, 11);
	}
...
void FrontendWarZ::eventOptionsReset(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	g_FastLoading->SetBool(true);

	int old_vsync = r_vsync_enabled->GetInt();
	r_vsync_enabled			->SetInt(0);

	//AlexRedd:: Extra options
	r_chat_message_sound->SetInt(0);//ChatSound
	r_crosshaire_mode->SetInt(0);//Crosshire mode
	r_highlight_mode->SetInt(0);//Highlight mode
	r_increase_fps->SetBool(true);//increaseFps
	r_disable_grass->SetBool(false);//disable grass
	r_disable_jump_sound->SetBool(true);//disable jump sound
	r_enable_alpha_sounds->SetBool(false);// Alpha music, sounds
	r_enable_killfeed_messages->SetBool(true);// KillFeed messages
	r_enable_killstreak_sounds->SetBool(true);// KillSreak sounds enable	
	r_killstreak_sounds->SetBool(false);// KillSreak sounds pack
	r_crosshaire_color->SetInt(0);//Crosshire color
	...
	{
		case 1:
			FillDefaultSettings( settings, S_WEAKV );
			settingsChangedFlags = SetDefaultSettings( S_WEAKV );
			break;
		case 2:
...
void FrontendWarZ::eventExtraOptionsApply(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(args);
	r3d_assert(argCount == 11);

	//AlexRedd:: Extra options
	r_chat_message_sound->SetInt((int)args[0].GetNumber() - 1);//Chat sound 	
	r_crosshaire_mode->SetInt((int)args[1].GetNumber() - 1);// Crosshire mode	
	r_highlight_mode->SetInt((int)args[2].GetNumber() - 1);// Highlight mode	
	r_increase_fps->SetInt((int)args[3].GetNumber() - 1);//Increase FPS option..		
	r_disable_grass->SetInt((int)args[4].GetNumber() - 1);//disable grass	
	r_disable_jump_sound->SetInt((int)args[5].GetNumber() - 1);//disable jump sound	
	r_enable_alpha_sounds->SetInt((int)args[6].GetNumber() - 1);// Alpha music, sounds	
	r_enable_killfeed_messages->SetInt((int)args[7].GetNumber() - 1);// KillFeed messages	
	r_enable_killstreak_sounds->SetInt((int)args[8].GetNumber() - 1);// KillSreak sounds enable	
	r_killstreak_sounds->SetInt((int)args[9].GetNumber() - 1);// KillSreak sounds
	r_crosshaire_color->SetInt((int)args[10].GetNumber() - 1);// Crosshire color
	
	int old_increase = r_increase_fps->GetInt();
	int old_disablegrass = r_disable_grass->GetInt();
	if (old_increase != r_increase_fps->GetInt() || old_disablegrass != r_disable_grass->GetInt())
	{
		// show message telling player that to change windows\fullscreen he have to restart game
		// todo: make fullscreen/window mode switch on the fly?
		Scaleform::GFx::Value var[2];
	...
	//AlexRedd:: cfgs for Increase Fps :]
	if (r_increase_fps->GetBool())
	{
		//r_grass_draw->SetBool(false);
		r_grass_anim_speed->SetBool(false);
		r_grass_ssao->SetBool(false);
		r_multithreading->SetBool(true);
		r_bloom->SetBool(false);
		r_shadows->SetBool(false);
		r_shadow_blur->SetBool(false);
		r_allow_hw_shadows->SetBool(false);
		r_optimize_shadow_map->SetBool(false);
		//r_half_res_particles->SetBool(false);
		r_split_grass_render->SetBool(false);
		r_ssao->SetBool(false);
		r_vsync_enabled->SetBool(false);
		r_particle_shadows->SetBool(false);
		r_disable_grass->SetInt(false);//disable grass	
	}
...
void FrontendWarZ::eventBrowseGamesJoin(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(gdata)
	{
		...
		if (gdata->info.flags & GBGameInfo::SFLAGS_ClanLimit)
			m_joinClanID = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].ClanID;
...
void FrontendWarZ::processNewGameList()
{
	...
	for(int i=0; i<numGames; i++) 
	{
		...
		var[14].SetBoolean((ginfo.flags & GBGameInfo::SFLAGS_Respawnfast) ? true : false); //Respawnfast
		var[16].SetBoolean((ginfo.flags & GBGameInfo::SFLAGS_NoDrop) ? true : false); //NARA::Nodrop
		var[17].SetBoolean((ginfo.flags & GBGameInfo::SFLAGS_DisableASR || ginfo.flags & GBGameInfo::SFLAGS_DisableSNP || ginfo.flags & GBGameInfo:: SFLAGS_MeleeOnly) ? true : false);
		if (ginfo.flags & GBGameInfo::SFLAGS_DisableASR)
			var[17].SetString("no_auto");
		else if (ginfo.flags & GBGameInfo::SFLAGS_DisableSNP)
			var[17].SetString("no_sniper");
		else
			var[17].SetString("no_icon");

		if (ginfo.mapId != GBGameInfo::MAPID_ServerTest)
		{
			gfxMovie.Invoke("_root.api.Main.BrowseGamesAnim.addGameToList", var, 18);
		}
		else {
			 if( gUserProfile.ProfileData.isDevAccount>0)
				 gfxMovie.Invoke("_root.api.Main.BrowseGamesAnim.addGameToList", var, 18);
...
void FrontendWarZ::eventCreateClan(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(strpbrk(clanCreateParams.ClanName, "!@#$%^&*()-=+_<>,./?'\":;|{}[] °¢£§•¶ß®©™´¨≠ÆØ∞±≤≥¥µ∂∑∏π∫ªºΩ æø¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“ ”‘’÷◊ÿŸ⁄‡·‚„‰ÂÊÁËÈÍÎÏÌÓ")!=NULL) // do not allow this symbols
	{
		Scaleform::GFx::Value var[2];
	...
	if(strpbrk(clanCreateParams.ClanTag, "!@#$%^&*()-=+_<>,./?'\":;|{}[] °¢£§•¶ß®©™´¨≠ÆØ∞±≤≥¥µ∂∑∏π∫ªºΩ æø¿¡¬√ƒ≈∆«»… ÀÃÕŒœ–—“ ”‘’÷◊ÿŸ⁄‡·‚„‰ÂÊÁËÈÍÎÏÌÓ")!=NULL) // do not allow this symbols
	{
		Scaleform::GFx::Value var[2];
...
void FrontendWarZ::eventRequestLotteryData(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(argCount == 0);
	...
	StartAsyncOperation(&FrontendWarZ::as_RequestLotteryThread, &FrontendWarZ::OnRequestLotterySuccess);	
}

unsigned int FrontendWarZ::as_RequestLotteryThread(void* in_data)
{
	r3dThreadAutoInstallCrashHelper crashHelper;
	FrontendWarZ* This = (FrontendWarZ*)in_data;
...
void FrontendWarZ::OnRequestLotterySuccess()
{
	gfxMovie.Invoke("_root.api.hideInfoMsg", "");	
	...
	for (size_t i = 0; i < gUserProfile.LotLog_.size(); i++)
	{
		...
		gfxMovie.Invoke("_root.api.Main.Marketplace.addLotteryData", args, 5);		
		gfxMovie.SetVariable("_root.api.Main.Marketplace.Marketplace.PopupLottery.Bank.Text.htmlText", Bank);
	...
	for (size_t i = 0; i < gUserProfile.WinLog_.size(); i++)
	{
		...
		gfxMovie.Invoke("_root.api.Main.Marketplace.addWinLotteryData", args, 3);
		//r3dOutToLog("!!!! addWinLotteryData: %s %s %d\n", strTime, strWUID, wl.Jackpot);
	}
	gUserProfile.WinLog_.clear();

	gfxMovie.Invoke("_root.api.Main.Marketplace.showLotteryPopup", "");
}
...
void FrontendWarZ::OnMyServerListSuccess()
{
	...
	for(size_t i=0; i<gUserProfile.serverRent->serverList.size(); i++)
	{
		...
		if (gClientLogic().m_gameInfo.channel == 5)		
			serverType = gLangMngr.getString("MapType_Stronghold");		
		else if (gClientLogic().m_gameInfo.channel == 6)		
			serverType = gLangMngr.getString("MapType_DevEvent");		
		else		
			serverType = gLangMngr.getString("MapType_Gameworld");
...
void FrontendWarZ::OnMyServerInfoSuccess()
{
...

	{
		...
		var[8].SetInt((srv->ginfo.flags & GBGameInfo::SFLAGS_MeleeOnly) ? 1 : 0);
		var[9].SetInt((srv->ginfo.flags & GBGameInfo::SFLAGS_Respawnfast) ? 1 : 0); //Respawnfast
		var[10].SetInt((srv->ginfo.flags & GBGameInfo::SFLAGS_NoDrop) ? 1 : 0); //NARA::Nodrop
		gfxMovie.Invoke("_root.api.Main.PlayGameMyServers.setServerInfoSettingsVisibility", var, 11);
	}
...
void FrontendWarZ::eventMyServerUpdateSettings(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	int meleeOnly = args[9].GetInt();
	int respawnFast = args[10].GetInt(); //Respawnfast
	int noDrop	  = args[11].GetInt(); //NARA::Nodrop
	...
	m_myServerFlags &= ~GBGameInfo::SFLAGS_MeleeOnly;
	m_myServerFlags &= ~GBGameInfo::SFLAGS_Respawnfast; //Respawnfast
	m_myServerFlags &= ~GBGameInfo::SFLAGS_NoDrop; //NARA::Nodrop
	...
	if(meleeOnly)    m_myServerFlags |= GBGameInfo::SFLAGS_MeleeOnly;
	if(respawnFast)   m_myServerFlags |= GBGameInfo::SFLAGS_Respawnfast; //Respawnfast
	if(noDrop)       m_myServerFlags |= GBGameInfo::SFLAGS_NoDrop; //NARA::Nodrop
...
void FrontendWarZ::eventBuyPremiumAccount(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	int PremiumValue = args[0].GetInt();

	if (PremiumValue == 1) {
        mStore_BuyItemID = 301257;
		mStore_BuyPrice = gUserProfile.ShopPremiumAcc7;
    } else if (PremiumValue == 2) {
        mStore_BuyItemID = 301256;
		mStore_BuyPrice = gUserProfile.ShopPremiumAcc15;
...
void FrontendWarZ::OnBuyPremiumSuccess()
{
	...
	int PremiumAcc = mStore_BuyItemID;

	if (PremiumAcc == 301257) {
        gUserProfile.ProfileData.PremiumAcc = 10080; // 7 days of premium
    } else if (PremiumAcc == 301256) {
        gUserProfile.ProfileData.PremiumAcc = 21600; // 15 days of premium
    } else if (PremiumAcc == 301259) {
...
void FrontendWarZ::eventRequestPinAccount(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(argCount == 0);

	// show pin dialog?
	gfxMovie.Invoke("_root.api.showSetPinWindow", "");
}

void FrontendWarZ::eventSetPinKeyAccount(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(argCount == 1);
	const char* pinCode = args[0].GetString();

	if (strlen(pinCode) != 6)
	{
		Scaleform::GFx::Value var[2];
		var[0].SetString(gLangMngr.getString("SafeLoginCodeMustContain"));
		var[1].SetBoolean(true);
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 2);
		return;
	}

	int apiCode = gUserProfile.ApiSetPinAccount(pinCode);
	const char* msg = "";
	if (apiCode == 0)
		msg = gLangMngr.getString("SetPin_Success");
	else if (apiCode == 7)
		msg = gLangMngr.getString("SetPin_ContainInvalidSymbols");
	else
		msg = gLangMngr.getString("SetPin_Failed");

	if (apiCode == 0)
	{
		g_PinCodeFirstStart->SetBool(false);

		gfxMovie.Invoke("_root.api.Main.OptionsGeneral.HidePin", false);
		//gfxMovie.SetVariable( "_root.api.Main.SurvivorsAnim.Survivors.BtnSetPin.visible", false );
	}
...
void FrontendWarZ::eventUnlockPinKeyAccount(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	r3d_assert(argCount == 1);
	const char* pinCode = args[0].GetString();
	...
	int apiCode = gUserProfile.ApiUnlockPinAccount(pinCode);
	const char* msg = "";
	if (apiCode == 0)
	{
		msg = gLangMngr.getString("SetPin_Success");

		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/PinLogin/Success"), r3dPoint3D(0, 0, 0));
	}
	else if (apiCode == 3)
		msg = gLangMngr.getString("SetPin_Block");
	else if (apiCode == 4)
	{
		// reload Get Bad Count
		if (gUserProfile.GetProfile() == 0)
		{
			char count[512];
	...
	if (apiCode == 0)
	{
		...
		gfxMovie.Invoke("_root.api.showPinScreen", false);
	}
