...
bool HUDDisplay::Init()
{
	...
	if(!gfxRangeFinder.Load("Data\\Menu\\WarZ_HUD_RangeFinder.swf", false))
		return false;
	if(!gfxFISH.Load("Data\\Menu\\WarZ_Fishing.swf", false)) 
		return false;
	...
	gfxFISH.SetCurentRTViewport(Scaleform::GFx::Movie::SM_ExactFit);
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDDisplay>(this, &HUDDisplay::FUNC)
	...
	gfxHUD.RegisterEventHandler("eventTaunt", MAKE_CALLBACK(eventTaunt));
	gfxHUD.RegisterEventHandler("eventBtnTauntSelect", MAKE_CALLBACK(eventBtnTauntSelect));
...
int HUDDisplay::Update()
{
	...
	//if (gClientLogic().m_gameInfo.channel == 6)// AlexRedd:: Do not show ThreatIndicator on channel 6 because we have a killfeed on this place
		//gfxHUD.SetVariable("_root.api.Main.ThreatIndicator.visible", 0);
	...
	if ((!hudKillFeedMsgQueue.empty() && !hudKillFeedMsgQueue2.empty() && !hudKillFeedMsgQueue3.empty()) && (r3dGetTime() - lastDisplayedHudKillFeedMessageTime)>1.5f)
	{
...
int HUDDisplay::Draw()
{
	...
	{
		R3DPROFILE_FUNCTION("gfxFISH.UpdateAndDraw");
			gfxFISH.UpdateAndDraw();
	}
...
void HUDDisplay::eventBtnTauntSelect(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	/*Scaleform::GFx::Value var[1];//ส่งค่าเมื่อเลือกท่าเต้นแล้ว กลับไปยัง Hud เพื่อทำอื่นๆอีก
	var[0].SetInt(TauntSelect);
	gfxHUD.Invoke("_root.Main.BtnTauntActive", var, 1);*/
}

void HUDDisplay::eventTaunt(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	//static float AddCoolDown = 0;
	float TimeCoolDown = r3dGetTime();
	obj_Player* plr = gClientLogic().localPlayer_;
	/* if (plr->NetworkLocal = true);
	{*/
	if (TauntSelect == 99)//ปิดเต้น
	{
		plr->m_TauntCoolDown = 0.0f;
		plr->TauntstartstopLittle = false;
		plr->TauntstartstopSpank = false;
		plr->TauntstartstopSpanktwo = false;
		plr->TauntstartstopHandFlip = false;
		plr->TauntstartstopJack = false;
		plr->TauntstartstopPThrust = false;
		plr->TauntstartstopBigPThrust = false;
		plr->TauntstartstopBalls = false;
		plr->PlayerState = PLAYER_IDLE;
		plr->SyncAnimation(true);
		showTaunt(0);
	}
	if (TimeCoolDown > plr->m_TauntCoolDown)
	{
		if (TauntSelect == 1)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 28.0f;
			plr->StartLitter();
			plr->TauntstartstopLittle = true;
			showTaunt(0);
		}
		else if (TauntSelect == 2)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 7.48f;
			plr->StartSpank();
			plr->TauntstartstopSpank = true;
			showTaunt(0);
		}
		else if (TauntSelect == 3)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 4.80f;
			plr->StartSpanktwo();
			plr->TauntstartstopSpanktwo = true;
			showTaunt(0);
		}
		else if (TauntSelect == 4)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 2.75f;
			plr->StartHandFlip();
			plr->TauntstartstopHandFlip = true;
			showTaunt(0);
		}
		else if (TauntSelect == 5)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 3.11f;
			plr->StartJack();
			plr->TauntstartstopJack = true;
			showTaunt(0);
		}
		else if (TauntSelect == 6)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 2.72f;
			plr->StartPThrust();
			plr->TauntstartstopPThrust = true;
			showTaunt(0);
		}
		else if (TauntSelect == 7)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 4.40f;
			plr->StartBigPThrust();
			plr->TauntstartstopBigPThrust = true;
			showTaunt(0);
		}
		else if (TauntSelect == 8)
		{
			plr->m_TauntCoolDown = TimeCoolDown + 3.82f;
			plr->StartBalls();
			plr->TauntstartstopBalls = true;
			showTaunt(0);
		}
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/NaraEvents/Misc/UI_Menu_Mystery_Box_Reveal"), gClientLogic().localPlayer_->GetPosition());
	}
	else
	{
...
void HUDDisplay::addChatMessage(int tabIndex, const char* user, const char* text, uint32_t flags)
{
	...
	bool isChatBanMsg = (flags&100)?true:false;//AlexRedd:: Ban chat
	...
	if(isChatBanMsg)//AlexRedd:: Ban chat
	{
		userColor = "#928f8f";
		textColor = "#928f8f";		
	}
...
void HUDDisplay::setSlotFish(int slotID, int quantity, const char* icon)
{
	if(!Inited) return;
	...
	gfxFISH.Invoke("_root.api.setSlot", var, 3);
}
...
void HUDDisplay::showBerFishing(int durability)
{
	if (!Inited) return;
	Scaleform::GFx::Value var[1];
	var[0].SetInt(durability);
	gfxFISH.Invoke("_root.api.showBerFishing", var, 1);
}
...
extern const char* getReputationIconName(int reputation);
void HUDDisplay::addPlayerToList(int index, int num, void* PlayerName_s_data)
{
	...
	const char* algnmtIcon = getReputationIconName(plrInfo->reputation);
	if(plrInfo->accountFlags&2) // if isDev, remove reputation string
		algnmt = "";
	var[3].SetString(algnmt);
	var[4].SetString(algnmtIcon);
	var[5].SetBoolean((plrInfo->accountFlags&1)?true:false);
	var[6].SetBoolean((plrInfo->accountFlags&2)?true:false);
	var[7].SetBoolean((plrInfo->accountFlags&4)?true:false);
	var[8].SetBoolean((plrInfo->groupFlags&1)?true:false);
	var[9].SetBoolean(plrInfo->voiceIsMuted);
	var[10].SetBoolean((plrInfo->accountFlags&8)?true:false);
	char tmpStr[256]={0};
	gClientLogic().localPlayer_->GetUserName(tmpStr);
	var[11].SetBoolean(strcmp(plrInfo->Gamertag, tmpStr)==0); // if this is local player
	var[12].SetInt(plrInfo->GamertagColor);
	gfxHUD.Invoke("_root.api.addPlayerToList", var, 13);
}
...
void HUDDisplay::showTaunt(int flag)
{
	if (!Inited) return;
	tauntVisible = flag;
	...
	Scaleform::GFx::Value var[1];
	var[0].SetBoolean(flag ? true : false);
	gfxHUD.Invoke("_root.api.showTaunt", var, 1);
...
void HUDDisplay::setRadWarning(bool rad)
{
	if (!Inited) return;
	gfxHUD.SetVariable("_root.api.Main.RadWarning.visible", rad);
}
...
void HUDDisplay::eventShowPlayerListContextMenu(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if(plrNamePtr) // it can be null if player left the game while localplayer kept player list open
	{
		...
		else if(!selectedSelf)
		{
			...
			{
				bool Clanlimit = gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_ClanLimit ? true : false;
				if (!Clanlimit)
		...
		if(isAdmin)
		{
			...
			if(gUserProfile.ProfileData.isDevAccount & wiUserProfile::DAA_TELEPORT)
				cntxCmds[curCntxCmd++] = ContextCommand(HUDDisplay_ContextMenu_AdminTeleportToPlayerID, "$HUD_PlayerAction_TELEPORT_TO");
			if(gUserProfile.ProfileData.isDevAccount & wiUserProfile::DAA_TELEPORT)
				cntxCmds[curCntxCmd++] = ContextCommand(HUDDisplay_ContextMenu_AdminTeleportPlayerToAdminID, "$HUD_PlayerAction_TELEPORT_TO_YOU");
		}
		else
		{
...
void HUDDisplay::showYouAreDead(const char* NickName, const char* TopText, const char* Pic)
{
	...
	respawnRequest = false; //Respawnfast
...
void HUDDisplay::updateDeadTimer(int timer) //Respawnfast
{
	if(!Inited) return;
...
void HUDDisplay::addCharTag(const char* name, bool isSameClan, int rep, int gamertagcolor, Scaleform::GFx::Value& result)
{
	if(!Inited) return;
	...
	var[3].SetInt(gamertagcolor);
	gfxHUD.Invoke("_root.api.addCharTag", &result, var, 4);
}
...
void HUDDisplay::addPlayerToGroupList(const char* name, bool isLeader, bool isLeaving, int GlowName, int Health)
{
	if(!Inited) return;
	Scaleform::GFx::Value var[5];
	...
	var[3].SetInt(GlowName);
	var[4].SetInt(Health);
	gfxHUD.Invoke("_root.api.addPlayerToGroup", var, 5);
}

void HUDDisplay::setGroupPlayerHealth(const char* name, int Health)
{
	if(!Inited) return;
	Scaleform::GFx::Value var[2];
	...
	var[0].SetString(name);
	var[1].SetInt(Health);
	gfxHUD.Invoke("_root.api.updatePlayerInGroup", var, 2);
}
...
void HUDDisplay::addHUDIcon(HUDIconType type, float lifetime, const r3dVector& pos, const char* markerDetail)
{
	if (!Inited)
		return;
	...
	var[1].SetString(markerDetail);
	gfxHUD.Invoke("_root.api.addScreenIcon", &m_HUDIcons[i].icon, var, 2);
...
void HUDDisplay::SetReloadingProgress(int progress)
{
	if (!Inited)
		return;
...
void HUDDisplay::showKillPlayer(const char* victim) //showKillPlayer
{
	if (!Inited) return;

	if(victim != "")
		gfxHUD.SetVariable("_root.api.Main.showKillPlayer.visible", true);
	else
		gfxHUD.SetVariable("_root.api.Main.showKillPlayer.visible", false);
...
void HUDDisplay::showSpawnProtect(int protect)
{
	if (!Inited) return;
	obj_Player* plr = gClientLogic().localPlayer_;	
	...
	else if(protect <= 0 && plr->CurLoadout.GameFlags & wiCharDataFull::GAMEFLAG_NearPostBox)
		gfxHUD.Invoke("_root.Main.Condition.gotoAndStop", "safe");
	else 
		gfxHUD.Invoke("_root.Main.Condition.gotoAndStop", "regular");

	gfxHUD.Invoke("_root.Main.Condition.ProtectBar.gotoAndStop", 30 - protect);
}
