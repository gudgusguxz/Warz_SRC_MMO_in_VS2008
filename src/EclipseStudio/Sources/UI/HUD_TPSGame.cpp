...
#include "UI\HUDSell.h"
#include "UI\HUDSellGC.h"
#include "UI\HUDExchange.h"
#include "UI\HUDCraftingNew.h"
...
HUDSell*	hudSell = NULL;
HUDSellGC*	hudSellGC = NULL;
HUDExchange* hudExchange = NULL;
HUDCraftingNew* hudCraftingNew = NULL;
HUDMark* hudMark = NULL;
...
void TPSGameHUD_OnStartGame()
{
	...
	hudSell = game_new HUDSell();
	hudSellGC = game_new HUDSellGC();
	hudExchange = game_new HUDExchange();
	hudCraftingNew = game_new HUDCraftingNew();
	hudMark = game_new HUDMark();
...
void TPSGameHUD :: DestroyPure()
{
	if(TPSGameHud_Inited)
	{
		...
		SAFE_DELETE(hudSell);
		SAFE_DELETE(hudSellGC);
		SAFE_DELETE(hudExchange);
		SAFE_DELETE(hudCraftingNew);
		SAFE_DELETE(hudMark);
	}
}
...
//	if(hudAttm->isActive()) return;//fixed bug 1267, (camera doesn't lerp hit "show attachment key" for the first time when attachments are active) 
	...
	if(hudSellGC->isActive()) return;
	if(hudExchange->isActive()) return;
	if(hudCraftingNew->isActive()) return;
...
static void DrawMenus()
{
	...
	*/
	...
	if (!win::bSuspended && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible() && !hudMain->isTauntVisible()/* && !plr->bDead*/)//ไม่สามารถกด I ได้เมื่อตาย
	{
		bool showHudPause = Keyboard->WasPressed(kbsEsc);
		if(!hudPause->isActive() || (hudPause->isActive() && !hudPause->isDisabledHotKeys()))
			showHudPause = showHudPause || InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_SWITCH_MINIMAP) || InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_INVENTORY) && gClientLogic().localPlayer_->AnglingStatus == 0;

		if(showHudPause && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive() && !hudSellGC->isActive() && !hudExchange->isActive() && !hudCraftingNew->isActive())
		{
			if(!hudPause->isActive())
			{
				...
				if(InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_SWITCH_MINIMAP))
					hudPause->showMap();
				else if(InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_INVENTORY) && gClientLogic().localPlayer_->AnglingStatus == 0)
		...
		//if((Keyboard->WasPressed(kbsEsc)) && hudVault->isActive() && !(gClientLogic().localPlayer_->m_isPlayerWaitRespawn))
		...
		if ((Keyboard->WasPressed(kbsEsc)) && hudSell->isActive())
			hudSell->Deactivate();

		if ((Keyboard->WasPressed(kbsEsc)) && hudSellGC->isActive())
			hudSellGC->Deactivate();

		bool showCraft = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_CRAFT_MENU);
		if(showCraft && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive() && !hudSellGC->isActive())
		{
		...
		if(gUserProfile.ProfileData.isDevAccount || gUserProfile.ProfileData.PremiumAcc) // @MRACOUSTIC #F10 STORE INGAME.
		{
			if(showStore && !hudAttm->isActive() && !hudVault->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudSell->isActive() && !hudSellGC->isActive())
			{
...
/*#ifdef ENABLE_INGAME_STORE
		bool showStore = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_STORE);
		if(gUserProfile.ProfileData.isDevAccount || gUserProfile.ProfileData.PremiumAcc)
		{
		if(showStore && !hudAttm->isActive() && !hudVault->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudSell->isActive())
		{
			if(!hudStore->isActive())
			{
				if(hudPause->isActive())
		...
		if((Keyboard->WasPressed(kbsEsc)) && hudExchange->isActive())
			hudExchange->Deactivate();
		...
		if((Keyboard->WasPressed(kbsEsc)) && hudCraftingNew->isActive())
			hudCraftingNew->Deactivate();
		...
		if( pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudSell->isActive() && !hudSellGC->isActive() && !hudExchange->isActive() && !hudCraftingNew->isActive())
		{
...
#endif
////////////////////////////////////////////////////
		bool showChatHUD = false;	
		if( pl && !hudCraft->isActive() && !hudAttm->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudPause->isActive() && !hudExchange->isActive() && !hudCraftingNew->isActive())
		{
			if( InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_TOGGLE_CHAT_HUD) )
			{				
				if( !hudMain->isChatHUDVisible() )
					hudMain->showChatHUD();
				else
					hudMain->hideChatHUD();				
			}			
			showChatHUD = hudMain->isChatHUDVisible();
		}
////////////////////////////////////////////////////
	}
	...
	if(hudCraftingNew->isActive())
	{
		r3dMouse::Show();

		R3DPROFILE_START( "hudCraftingNew->" );

		hudCraftingNew->Update();
		hudCraftingNew->Draw();

		R3DPROFILE_END( "hudCraftingNew->" );
	...
	if (hudExchange->isActive())
	{
		r3dMouse::Show(); // make sure that mouse is visible

		R3DPROFILE_START("hudExchange->");

		hudExchange->Update();
		hudExchange->Draw();

		R3DPROFILE_END("hudExchange->");
	...
	if (hudSell->isActive())
	{
		...
		R3DPROFILE_START("hudSell->");

		hudSell->Update();
		hudSell->Draw();

		R3DPROFILE_END("hudSell->");

		return;
	}

	if (hudSellGC->isActive())
	{
		r3dMouse::Show(); // make sure that mouse is visible

		R3DPROFILE_START("hudSellGC->");

		hudSellGC->Update();
		hudSellGC->Draw();

		R3DPROFILE_END("hudSellGC->");
	...
	{
		if (ChatWindowSwitch && hudMain && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible() && !hudMain->isTauntVisible() && r_render_in_game_HUD->GetBool())
		{
		...
		if(hudMain)
		{				
			if(InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_SHOW_FRIENDS_NAME))//AlexRedd:: For new friend name button			
			   r_render_friends_name->SetBool(!r_render_friends_name->GetBool());
		}

		if (hudMain && !hudMain->isChatInputActive() && !hudMain->isPlayersListVisible() && !hudMain->isTauntVisible())
		{
		...
		if (hudMain && !hudMain->isChatInputActive())//AlexRedd:: improved!
		{
			bool tabPressed = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_PLAYERS);
			bool tabReleased = InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_SHOW_PLAYERS);
			bool escPressed = Keyboard->WasPressed(kbsEsc);
			...
			else if ((tabReleased || escPressed) && hudMain->isPlayersListVisible())
				hudMain->showPlayersList(0);
		}

		if (hudMain && !hudMain->isChatInputActive() && !pl->bDead)//Taunt
		{
			bool tabPressed = InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SHOW_TAUNT);
			bool tabReleased = InputMappingMngr->wasReleased(r3dInputMappingMngr::KS_SHOW_TAUNT);
			bool escPressed = Keyboard->WasPressed(kbsEsc);
			if (tabPressed && !hudMain->isTauntVisible())
			{
				hudMain->showTaunt(1);
				hudTrade->requestClose();//ปิดเทรด
			}
...

		{
			...
			if (!pl->bDead && !hudAttm->isActive() && !hudPause->isActive() && !hudActionUI->isActive() && !hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive())
			{
				R3DPROFILE_START("hudMark->");
				hudMark->Update();
				hudMark->Draw();
				R3DPROFILE_END("hudMark->");
			}
			...
			if(!pl->bDead && (r3dGetTime() - pl->TimeOfLastRespawn)>300.0f && !hudAttm->isActive() && !hudPause->isActive() && !hudActionUI->isActive() && 
				!hudVault->isActive() && !hudStore->isActive() && !hudSafelock->isActive() && !hudTrade->isActive() && !hudRepair->isActive() && !hudCraft->isActive() && !hudSell->isActive() && !hudSellGC->isActive())
			{
...
// 				}
		...
		if (hudMain && (hudMain->isChatInputActive() || hudMain->isPlayersListVisible() || hudMain->isTauntVisible())) // also checks for write note, so do not hide mouse
			return;
...
void ProcessPlayerMovement(obj_Player* pl, bool editor_debug )
{
	...
	if( !g_bEditMode )
	{
		...
		if (pl->IsInVehicle())
		{
			...
			if (hudMain->isTauntVisible()) return;
			if(hudPause->isActive()) return;
			if(hudVault->isActive()) return;
			if(hudRepair->isActive()) return;
			if(hudCraft->isActive()) return;
	...
	if (InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_FPS_TPS) && !(hudAttm && hudAttm->isActive()) && !(hudMain && (hudMain->isChatInputActive() || hudMain->isPlayersListVisible() || hudMain->isTauntVisible())) && !Mouse->GetMouseVisibility() && !stillPlayingProningAnim)
	{
		pl->switchFPS_TPS();
	}
	...
	if(
		...
		pl->IsInVehicle() ||
#endif
		Mouse->GetMouseVisibility() || (hudMain && (hudMain->isChatInputActive() || hudMain->isPlayersListVisible() || hudMain->isTauntVisible()))) // do not update player if we are in menu control mode!
	{
...
#ifdef FINAL_BUILD
	if (disablePlayerMovement && (hudPause->isActive() || hudMain->isPlayersListVisible() || hudMain->isTauntVisible() || hudCraft->isActive() || hudAttm->isActive())) //TheHexa:: Players disconnecting can't move anymore
	{
	...
	if (!g_bEditMode)
	{
		if (disablePlayerMovement && (hudPause->isActive() || hudMain->isPlayersListVisible() || hudMain->isTauntVisible())) //TheHexa:: Players disconnecting can't move anymore
		{
			if (!hudPause->disableHotKeys && !hudPause->isDisconnecting_)
			{
				disablePlayerMovement = false;
	...
	//TheHexa:: Fix player rotation in spectator mode
	...
	if(!pl->m_isInScope)
	{
		if(InputMappingMngr->wasPressed(r3dInputMappingMngr::KS_SWITCH_TPS_CAMERA_SIDE) && !(hudVault && hudVault->isActive()) && 
			...
			!(hudSafelock && hudSafelock->isActive()) && !(hudSell && hudSell->isActive()) && !(hudSellGC && hudSellGC->isActive()) && !(hudExchange && hudExchange->isActive()) && 
			!(hudCraftingNew && hudCraftingNew->isActive()))//cynthia: 1785
		{
	...
	VMPROTECT_BeginMutation("ProcessPlayerMovement_Accel");	
	{
		...
		if(!disablePlayerMovement)
		{
			...
			if(aiming || pl->m_isHoldingBreath || pl->CurLoadout.GameFlags & wiCharDataFull::GAMEFLAG_RadioactiveArea) // cannot spring and aim. also, in default key binding spring and hold breath are on the same key
				shiftWasPressed = 0;
			...
			if(pl->CurLoadout.HeroItemID == 20228 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20229 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20230 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20232 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20233 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20234 ) //ตัวละครวิ่งไว #ตัวสั่งทำ
			accelaration *= 1.2f;

			if(pl->CurLoadout.HeroItemID == 20226 ) //ตัวละครวิ่งไว #โปร50
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20255 ) //ตัวละครวิ่งไว #โปร50
			accelaration *= 1.2f;
			if(pl->CurLoadout.HeroItemID == 20253 ) //ตัวละครวิ่งไว #โปร50
			accelaration *= 1.2f;

			if(pl->CurLoadout.HeroItemID == 20225 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20240 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20241 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20244 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20245 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20224 ) //ตัวละครวิ่งไว #3500GC
			accelaration *= 1.1f;

			if(pl->CurLoadout.HeroItemID == 20222 ) //กิจกรรม
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20223 ) //กิจกรรม
			accelaration *= 1.1f;
			if(pl->CurLoadout.HeroItemID == 20212 ) //กิจกรรม
			accelaration *= 1.1f;


			if(pl->CurLoadout.HeroItemID == 20252 ) //ตัวละครวิ่งไว #3ตัวบาท
			accelaration *= 1.3f;
			if(pl->CurLoadout.HeroItemID == 20254 ) //ตัวละครวิ่งไว #3ตัวบาท
			accelaration *= 1.3f;
			
#ifdef FINAL_BUILD
			if(gUserProfile.ProfileData.isDevAccount && Keyboard->IsPressed(kbsV))			
				accelaration *= 8.0f;		
			if(gUserProfile.ProfileData.isDevAccount && Keyboard->IsPressed(kbsX))
                pl->JumpVelocity = 17; 
#else
			if(g_bEditMode && Keyboard->IsPressed(kbsLeftAlt))			
				accelaration *= 8.0f;			
			else if(!g_bEditMode && Keyboard->IsPressed(kbsLeftAlt))			
				accelaration *= 8.0f;			
#endif
...
//----------------------------------------------------------------
{
	...
	bool allow_specator_mode = gUserProfile.ProfileData.isDevAccount > 0;
#endif
