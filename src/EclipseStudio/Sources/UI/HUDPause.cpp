...
void HUDPause::ChangePlayerBackpackFast(int ItemID)
{
	...
	if(slotFrom == -1)
		return;
	if(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop) //NARA::Nodrop
		return;
...
void HUDPause::eventShowContextMenuCallback(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	{		
		if (gClientLogic().m_gameInfo.channel != 6 && !(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop)) //NARA::Nodrop
		{			
	...
	{	if(itm->m_itemID!=WeaponConfig::ITEMID_Binoculars && itm->m_itemID!=WeaponConfig::ITEMID_RangeFinder && itm->m_itemID != WeaponConfig::ITEMID_FishingRod && itm->m_itemID != WeaponConfig::ITEMID_FishingRod1 && itm->m_itemID != WeaponConfig::ITEMID_SpawnCra
			&& itm->m_itemID!=WeaponConfig::ITEMID_RepairKit && itm->m_itemID!=WeaponConfig::ITEMID_PremRepairKit && itm->category != storecat_BoxReady)
		{
	...
	if (itm->category == storecat_BoxReady)
	{
		var[0].SetString("$FR_OPEN_BoxReadyPlay");
		var[1].SetInt(HPA_OPEN_BoxReady);
		gfxMovie.Invoke("_root.api.Main.Inventory.addContextMenuOption", var, 2);
	}
	...
	if( !plr->IsSwimming() && !(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop) && itm->category != storecat_HeroPackage) //NARA::Nodrop
	{
	...
	if (!plr->IsSwimming() && wi.quantity > 1 && !(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop)&& itm->category != storecat_HeroPackage)//AlexRedd:: drop all //NARA::Nodrop
	{
...
#ifndef FINAL_BUILD
	if( !plr->IsSwimming() && plr->CurLoadout.Skills[CUserSkills::SKILL_Repair3]
	&& !(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop))//AlexRedd:: Destroy item with skill //NARA::Nodrop
	{
...
void HUDPause::eventDragDrop(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount) //drag drop
{
	if (gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop)
		return;
    ...
    if (actionID == HPA_DRAG_DROP_ITEM)//NARA:DRAGDROP
	{
...
void HUDPause::eventContextMenu_Action(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	else if (actionID == HPA_OPEN_BoxReady)
	{
		wiInventoryItem& wi = plr->CurLoadout.Items[slotID];
		...
		if (item)
		{
			if (item->category == storecat_BoxReady)
			{
				plr->OpenBoxReadyPlay(slotID);
			}
		}
	...
	else if(actionID == HPA_USE_ITEM)
	{
		...
		else if(wi.itemID == WeaponConfig::ITEMID_AirHorn)
		{
			...
			wi.itemID == WeaponConfig::ITEMID_RiotShieldTM ||
			wi.itemID == WeaponConfig::ITEMID_RiotShieldGalax ||
			wi.itemID == WeaponConfig::ITEMID_RiotShieldJumper ||

			wi.itemID == WeaponConfig::ITEMID_Lockbox ||
			wi.itemID == WeaponConfig::ITEMID_LockboxGALAX ||
			wi.itemID == WeaponConfig::ITEMID_LockboxTM ||
			wi.itemID == WeaponConfig::ITEMID_LockboxTT ||
			wi.itemID == WeaponConfig::ITEMID_LockboxNKS ||
			wi.itemID == WeaponConfig::ITEMID_LockboxDELL ||
			wi.itemID == WeaponConfig::ITEMID_LockboxREDBULL ||
			wi.itemID == WeaponConfig::ITEMID_LockboxGIG ||
			wi.itemID == WeaponConfig::ITEMID_LockboxARE ||
			wi.itemID == WeaponConfig::ITEMID_LockboxPIG ||
			wi.itemID == WeaponConfig::ITEMID_LockboxArmy ||
		...
		else
		{
			...
			if((wi.itemID == WeaponConfig::ITEMID_BOXRANDOM1 || //RandomBox
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM2 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM3 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM4 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM5 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM6 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM7 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM8 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM9 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM10 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM11 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM12 ||
				wi.itemID == WeaponConfig::ITEMID_BOXRANDOM13)
				)
			{
				if((r3dGetTime() - plr->lastTimeUsedConsumable) < plr->currentConsumableCooldownTime)
				{
...
{
	if(gClientLogic().m_gameInfo.flags & GBGameInfo::SFLAGS_NoDrop) //TESTBUG
	{
		return ;
	}
	else
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDPause>(this, &HUDPause::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventDragDrop", MAKE_CALLBACK(eventDragDrop)); //drag drop
...
void HUDPause::Activate()
{
	...
	//new opt
	var[30].SetNumber( r_crosshaire_mode->GetInt() + 1);//Crosshair mode
	var[31].SetNumber( r_highlight_mode->GetInt() + 1);//Highlight mode
	var[32].SetNumber( r_chat_message_sound->GetInt() + 1);
	var[33].SetNumber( r_disable_jump_sound->GetInt() + 1); //Disable jump sound
	var[34].SetNumber( r_enable_killstreak_sounds->GetInt() + 1);// KillSreak sounds enable
	var[35].SetNumber( r_killstreak_sounds->GetInt() + 1);// KillSreak sounds pack (0-crossfire, 1-quake, Duke)
	var[36].SetNumber( r_crosshaire_color->GetInt() + 1);// Crosshaire color (0 ="white", 1 ="red", 2 ="blue", 3 ="green")
	var[37].SetNumber( r_enable_alpha_sounds->GetInt()+1);// Alpha music, sounds
...
void HUDPause::eventOptionsControlsReset(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	r_chat_message_sound->SetInt(0);//ChatSound Reset this option	
...
//	r_contrast				->SetFloat( R3D_CLAMP((float)args[2].GetNumber(), 0.25f, 0.75f) );
	...
	r_crosshaire_mode->SetInt((int)args[12].GetNumber() - 1);// Crosshire mode	
	r_highlight_mode->SetInt((int)args[13].GetNumber() - 1);// Highlight mode	
	r_chat_message_sound->SetInt((int)args[14].GetNumber() - 1);//Chat sound 	
	r_disable_jump_sound->SetInt((int)args[15].GetNumber() - 1);//disable jump sound	
	r_enable_killstreak_sounds->SetInt((int)args[16].GetNumber() - 1);// KillSreak sounds enable	
	r_killstreak_sounds->SetInt((int)args[17].GetNumber() - 1);// KillSreak sounds
	r_crosshaire_color->SetInt((int)args[18].GetNumber() - 1);// Crosshire color
	r_enable_alpha_sounds->SetInt((int)args[19].GetNumber() - 1);// Alpha music, sounds	
