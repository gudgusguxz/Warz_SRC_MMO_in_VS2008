...
void HUDTrade::eventGameDollarsToTrade(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount) //TradeGD
{
	// gridFrom:int, TradeGridTo:int, amount:int
	r3d_assert(argCount==1);
	int gamedollar = args[0].GetInt();
	
	if(isCloseRequested)
		return;

	//@tobi check gamedollars if bigger then you have give out error msg
	if(gamedollar > gUserProfile.ProfileData.GameDollars) {
		Scaleform::GFx::Value var[3];
		var[0].SetString("Not enough GameDollars!");
		var[1].SetBoolean(true);
		var[2].SetString("");
		gfxMovie.Invoke("_root.api.showInfoMsg", var, 3);
		gfxMovie.Invoke("_root.api.Main.fixGameDollarText", gUserProfile.ProfileData.GameDollars);
		return;
	}
	else if(gamedollar < 0) {
		Scaleform::GFx::Value var[3];
		var[0].SetString("Not enough Gamedollars!");
		var[1].SetBoolean(true);
		var[2].SetString("");
...
#define MAKE_CALLBACK(FUNC) game_new r3dScaleformMovie::TGFxEICallback<HUDTrade>(this, &HUDTrade::FUNC)
	...
	gfxMovie.RegisterEventHandler("eventGameDollarsToTrade", MAKE_CALLBACK(eventGameDollarsToTrade)); //TradeGD
...
void HUDTrade::Deactivate()
{
	if(!isCloseRequested && (!isOppositeIndicatorOn || !isUserIndicatorOn))
	{
		...
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/UI_Trade_Cancel"), gClientLogic().localPlayer_->GetPosition());
	}
...
void HUDTrade::Activate(const char* plrNameToTradeWith)
{
	...
	// set opposite player name
	{
		...
		gfxMovie.Invoke("_root.api.Main.SetGD", gUserProfile.ProfileData.GameDollars); //TradeGD
	}
...
void HUDTrade::setOfferGD(int Gamedollars) //TradeGD
{
	gfxMovie.Invoke("_root.api.Main.SetGameDollar", Gamedollars);
	reloadTradeInfo();
	updateSurvivorTotalWeight();
	gfxMovie.Invoke("_root.api.updateBackpackAndTradeWindows", "");
