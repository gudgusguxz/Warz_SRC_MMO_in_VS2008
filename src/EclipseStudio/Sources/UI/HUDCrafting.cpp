...
void HUDCrafting::eventCraftItem(r3dScaleformMovie* pMovie, const Scaleform::GFx::Value* args, unsigned argCount)
{
	...
	if (gClientLogic().localPlayer_)
		SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/User_Action/User_Action_Craft_Item_v2"),gClientLogic().localPlayer_->GetPosition());
}
...
void HUDCrafting::Deactivate()
{
	...
	//if(gClientLogic().localPlayer_->m_isPlayerWaitRespawn)
	//{
	//	PKT_C2S_addProtection_s n;			
	//	n.PlayerID = toP2pNetId(gClientLogic().localPlayer_->GetNetworkID());
	//	p2pSendToHost(gClientLogic().localPlayer_, &n, sizeof(n), true);
	//	gClientLogic().localPlayer_->m_isPlayerWaitRespawn = false;
	//}
