...
void MasterServerLogic::OnNetData(DWORD peerId, const r3dNetPacketHeader* packetData, int packetSize)
{
  ...
  {
    r3dOutToLog("MasterNet: packet too small (%d), dropping\n", packetSize);
    return;
  }
  if(net_ == NULL)
    return;
  ...
  {
    default:
    r3dOutToLog("MasterNet: invalid packetId %d size %d\n", packetData->EventID, packetSize);
      return;
      
    case GBPKT_M2C_MasterInfo:
    {
    if(packetSize < (int)sizeof(GBPKT_M2C_MasterInfo_s))
    {
      r3dOutToLog("MasterNet: bad MasterInfo size %d\n", packetSize);
      return;
    }
    ...
    {
    if(packetSize < (int)sizeof(GBPKT_ValidateConnectingPeer_s))
    {
      r3dOutToLog("MasterNet: bad ValidateConnectingPeer size %d\n", packetSize);
      return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad ShutdownNote size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad StartGamesList size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad SupervisorData size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad GameData size %d\n", packetSize);
    return;
    }
      //r3dOutToLog("GBPKT_M2C_GameData\n");
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad EndGamesList size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad JoinGameAns size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad MyServerAddPlayer size %d\n", packetSize);
    return;
    }
    ...
    if(packetSize != (int)sizeof(n))
    {
    r3dOutToLog("MasterNet: bad MyServerInfoAns size %d\n", packetSize);
    return;
    }
...
void MasterServerLogic::RequestGameList(const NetPacketsGameBrowser::GBPKT_C2M_RefreshList_s& n)
{
  ...
  {
    r3dOutToLog("MasterNet: RequestGameList while not connected\n");
    return;
  }
  if(badClientVersion_) 
  {
    r3dOutToLog("MasterNet: RequestGameList blocked (bad client version)\n");
    return;
  }
...
int MasterServerLogic::WaitForGameList()
{
  ...
  {
    r3dOutToLog("MasterNet: can't receive game list (timeout)\n");
    return 0;
  }
...
void MasterServerLogic::SendJoinGame(DWORD gameServerId, int clanID, const char* pwd)
{
  if(!net_ || !net_->IsConnected())
  {
    r3dOutToLog("MasterNet: SendJoinGame while not connected\n");
    return;
  }
  if(badClientVersion_) 
  {
    r3dOutToLog("MasterNet: SendJoinGame blocked (bad client version)\n");
    return;
  }
...
void MasterServerLogic::SendJoinQuickGame(const NetPacketsGameBrowser::GBPKT_C2M_QuickGameReq_s& n)
{
  if(!net_ || !net_->IsConnected())
  {
    r3dOutToLog("MasterNet: SendJoinQuickGame while not connected\n");
    return;
  }
  if(badClientVersion_) 
  {
    r3dOutToLog("MasterNet: SendJoinQuickGame blocked (bad client version)\n");
    return;
  }
