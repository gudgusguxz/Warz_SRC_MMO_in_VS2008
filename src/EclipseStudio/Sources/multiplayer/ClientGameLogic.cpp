...
void ClientGameLogic::OnNetData(DWORD peerId, const r3dNetPacketHeader* packetData, int packetSize)
{
	...
	if(packetSize < (int)sizeof(DefaultPacket))
	{
		r3dOutToLog("OnNetData: packet too small (%d), dropping\n", packetSize);
		return;
	}
	...
	{
		r3dOutToLog("OnNetData: drop event %d from missing object %d\n", evt->EventID, evt->FromID);
		return; 
	}
	...
	if(fromObj) 
	{
		...
		if(!fromObj->OnNetReceive(evt->EventID, packetData, packetSize)) 
			r3dOutToLog("OnNetData: handler failed for event %d obj %s\n", evt->EventID, fromObj->Class->Name.c_str());
		return;
	}

	r3dOutToLog("OnNetData: unknown world event %d\n", evt->EventID);
	return;
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_JoinGameAns)
{
	...
	//GetRektBambi tutorial
	if(m_gameInfo.mapId != GBGameInfo::MAPID_WZ_Trade_Map && m_gameInfo.mapId != GBGameInfo::MAPID_ServerTest //Only set time of day if mapid is not day map 
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP01
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP02
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP03
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP04
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP05
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP06
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP07
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP08
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP09
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP10
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP11
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP12
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP13
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP14
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP15
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP16
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP17
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP18
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP19
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP20
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP21
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP22
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP23
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP24
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP25)
		UpdateTimeOfDay();
	else
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_C2C_MarkerData)
{
	obj_Player* fromPl = (obj_Player*)fromObj;
	hudMark->showHUDIcon(fromPl, n.id, n.pos, n.markerdetail);
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_C2C_ChatMessage)
{
	...
	if(r_chat_message_sound->GetBool() && gClientLogic().localPlayer_)
	{  		
		if (n.msgChannel == 3)//group
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/UI_Group_Message_Recieved"), gClientLogic().localPlayer_->GetPosition());
		else
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/UI_Confirm"), gClientLogic().localPlayer_->GetPosition());
	}
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_CreateNetObject)
{
	if(n.itemID == WeaponConfig::ITEMID_BarbWireBarricade ||
		...
		n.itemID == WeaponConfig::ITEMID_RiotShieldTM ||
		n.itemID == WeaponConfig::ITEMID_RiotShieldGalax ||
		n.itemID == WeaponConfig::ITEMID_RiotShieldJumper ||
	...
	else if(n.itemID == WeaponConfig::ITEMID_FarmBlock || n.itemID == WeaponConfig::ITEMID_SolarWaterPurifier)
	{
		...
		block->OnCreate();
			...
			n.itemID == WeaponConfig::ITEMID_LockboxGALAX ||
			n.itemID == WeaponConfig::ITEMID_LockboxTT ||
			n.itemID == WeaponConfig::ITEMID_LockboxNKS ||
			n.itemID == WeaponConfig::ITEMID_LockboxDELL ||
			n.itemID == WeaponConfig::ITEMID_LockboxREDBULL ||
			n.itemID == WeaponConfig::ITEMID_LockboxGIG ||
			n.itemID == WeaponConfig::ITEMID_LockboxARE ||
			n.itemID == WeaponConfig::ITEMID_LockboxPIG ||
			n.itemID == WeaponConfig::ITEMID_LockboxArmy ||
			n.itemID == WeaponConfig::ITEMID_LockboxTM )
	{
		...
		safelock->OnCreate();
			...
			n.itemID == WeaponConfig::ITEMID_ZombieGrenade ||
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_PlayerNameJoined)
{
	...
	playerNames[n.playerIdx].GamertagColor = n.gamertagcolor;
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_RespawnPlayer) //Respawnfast
{
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_addProtectionAns)
{
	GameObject* targetObj = fromObj;
	r3d_assert(targetObj->isObjType(OBJTYPE_Human));
	obj_Player* targetPlr = (obj_Player*)targetObj;

	targetPlr->addProtect(n.spawnProtection);	
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_CallForHelpEvent)
{
	...
	if(hudMain)
	{
		...
		if (gClientLogic().localPlayer_)
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/UI_CallForHelp"),gClientLogic().localPlayer_->GetPosition());
	}
}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_CreatePlayer)
{
	...
	if(n.playerIdx == localPlayerIdx_)
	{
		...
		if(gClientLogic().m_gameInfo.channel == 6 /*&& m_gameInfo.gameServerId!=22000*/)// 22000 = no drop (sinch point client/server)
		{
			wipeInventory=true;
		...
		if(hudMain)
		{
			...
			case 6:
				...
				break;
			case 7:
				r3dscpy(tmpStr, (gLangMngr.getString("JoinedTradeServer"))); 
				break;
			default:
			...
			if(m_gameInfo.flags & GBGameInfo::SFLAGS_MeleeOnly)
			{
				hudMain->addChatMessage(0, "<SYSTEM>", gLangMngr.getString("JoinedServerWithMeleeRestriction"), 0);
			}
#ifdef DISABLE_GI_ACCESS_ON_PTE_STRONGHOLD_MAP
			if(m_gameInfo.channel == 6 && m_gameInfo.mapId==GBGameInfo::MAPID_WZ_Cliffside) // PTE map, wipe inventory and give one flashlight only
		...
		{
			//AlexRedd:: Fix DEV EVENT SERVERS
			{
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_Damage)
{
	...
	if(targetObj->isObjType(OBJTYPE_Human))
	{
		...
		if( hudMain )
		{
			if(fromObj != localPlayer_)
			{
				obj_Player* tarketPlr = (obj_Player*)fromObj;
				const WeaponConfig* wcfg = g_pWeaponArmory->getWeaponConfig( tarketPlr->m_Weapons[tarketPlr->m_SelectedWeapon]->getItemID() );
                if(tarketPlr && wcfg && wcfg->m_effectDmgDance )
				{
                    tarketPlr->StartDamageDance();
				}
			}
		}
	}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_KillPlayer)
{
	...
	targetPlr->DoDeath(killedID, n.forced_by_server, (STORE_CATEGORIES)n.killerWeaponCat, n.lastTimeHitItemID, n.KillFlag);
	
	if(n.forced_by_server)
		return;

	///////////////////////////////////////////////////////////////// KillFeed - KillSreak here
	
	//if (m_gameInfo.channel == 6)
	//{
		obj_Player* plr = localPlayer_;
		if (hudMain)
		{
			if (r_enable_killfeed_messages->GetBool())
			{
				char victim[64] = {0};
				char damageType[64] = {0};
				char killer[64] = {0};
				char reason[64] = {0};
				...
				{
				case 1:
					strcpy_s(reason, 64, "suicide");
					break;
				case 2:
					strcpy_s(reason, 64, "headshot");
					break;
				case 3:
					strcpy_s(reason, 64, "unarmed");
					break;
				case 4:
					switch (n.killerWeaponCat)
					{
					case storecat_ASR:
						strcpy_s(reason, 64, "asr");
						break;
					case storecat_SNP:
						strcpy_s(reason, 64, "snp");
						break;
					case storecat_SHTG:
						strcpy_s(reason, 64, "shtg");
						break;
					case storecat_MG:
						strcpy_s(reason, 64, "mg");
						break;
					case storecat_HG:
						strcpy_s(reason, 64, "hg");
						break;
					case storecat_SMG:
						strcpy_s(reason, 64, "smg");
						break;
					case storecat_GRENADE:
						strcpy_s(reason, 64, "grenade");
						break;
					case storecat_MELEE:
						strcpy_s(reason, 64, "melee");
						break;
					default:
						break;
					}
			...
			// Killstreaks & Sounds
			bool isRussian = strcmp(g_user_language->GetString(), "russian") == 0 ? true : false;//russian client?
			bool HotKills = false;//Spetial kills (sounds)
			int KillStreak = n.Killstreak;// For Killstreak HUD

			if (killerObj->NetworkLocal)
			{
				if (r_enable_killstreak_sounds->GetBool())//Enable sounds?
				{
					if (n.KillFlag > 1 && n.KillFlag < 5)
					{
						switch (n.KillFlag)
						{
						case 2://Headshot kill
							HotKills = true;
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/Headshot" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/Headshot"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/Headshot" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/Headshot"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/headshot"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/Headshot"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/Headshot"), plr->GetPosition());
								}
							}
							break;
						case 3://By Player (Unarmed Melee)
							HotKills = true;
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/Knifekill" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/Knifekill"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/Knifekill" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/Knifekill"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake (CroosFire)
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
									"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/Knifekill" :
							...
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								...
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/Knifekill"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/Knifekill"), plr->GetPosition());
								}
							}
							break;
						case 4://For weapon spetal kills
							if (n.killerWeaponCat == storecat_GRENADE)//Grenade kill
							{
								HotKills = true;
								if (r_killstreak_sounds->GetInt() == 0)//CroosFire
								{
									if (isRussian)
									{
										if (plr)
											SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
											"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/Grenadekill" :
											"Sounds/UI Events/KillStreak/CrossFire/RU/Male/Grenadekill"), plr->GetPosition());
									}
									else
									{
										if (plr)
											SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
											"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/Grenadekill" :
											"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/Grenadekill"), plr->GetPosition());
									}
								}
								else if (r_killstreak_sounds->GetInt() == 1)//Quake (CroosFire)
								{
								...
								else if (r_killstreak_sounds->GetInt() == 2)//Duke
								{
									...
									{
										if (plr)
											SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/Grenadekill"), plr->GetPosition());
									}
									else
									{
										if (plr)
											SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/Grenadekill"), plr->GetPosition());
									}
								}
							}
							else if (n.killerWeaponCat == storecat_MELEE)// Melee kill
							{
								HotKills = true;
								if (r_killstreak_sounds->GetInt() == 0)//CroosFire
								{
						...
						{							
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_2" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_2"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_2" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_2"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/doublekill"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_2"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_2"), plr->GetPosition());
								}
							}
						}
						...
						{							
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_3" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_3"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_3" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_3"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/triplekill"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_3"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_3"), plr->GetPosition());
								}
							}
						}
						...
						{							
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_4" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_4"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_4" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_4"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/multikill"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_4"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_4"), plr->GetPosition());
								}
							}
						}
						...
						{							
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_5" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_5"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_5" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_5"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/megakill"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_5"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_5"), plr->GetPosition());
								}
							}
						}
						...
						{							
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_6" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_6"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_6" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_6"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/monsterkill"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_6"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_6"), plr->GetPosition());
								}
							}
						}
						else if (KillStreak == 7)
						{
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_7" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_7"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_7" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_7"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/holyshit"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_7"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_7"), plr->GetPosition());
								}
							}
						}
						else if (KillStreak >= 8)
						{
							if (r_killstreak_sounds->GetInt() == 0)//CroosFire
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/RU/Famale/MultiKill_8" :
										"Sounds/UI Events/KillStreak/CrossFire/RU/Male/MultiKill_8"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath(plr->m_isFemaleHero ?
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Famale/MultiKill_8" :
										"Sounds/UI Events/KillStreak/CrossFire/ENG/Male/MultiKill_8"), plr->GetPosition());
								}
							}
							else if (r_killstreak_sounds->GetInt() == 1)//Quake
							{
								if (plr)
									SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/Quake/thats_the_way"), plr->GetPosition());
							}
							else if (r_killstreak_sounds->GetInt() == 2)//Duke
							{
								if (isRussian)
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/RU/MultiKill_8"), plr->GetPosition());
								}
								else
								{
									if (plr)
										SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/KillStreak/DukeNukem/ENG/MultiKill_8"), plr->GetPosition());
								}
							}
						}
					}
				}
			   hudMain->showKillStreak(KillStreak);
			   
			   //showKillPlayer
				if(n.KillFlag != 1)
					hudMain->showKillPlayer(n.victim);
				//
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_GroupAns)
{
	if(hudMain)
	{
		...
		case PKT_S2C_GroupAns_s::GSE_SuccessSentInvite:
			...
			SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/UI_Group_Invite_Sent"), gClientLogic().localPlayer_->GetPosition());
			break;
	...
	if(hudMain)
	{
		...
		for(int i=0; i<R3D_ARRAYSIZE(playerNames); ++i)
		{
			if(playerNames[i].Gamertag[0] && stricmp(playerNames[i].Gamertag, n.gamertag)==0)
			{
				...
				if (gClientLogic().localPlayer_)
					SoundSys.PlayAndForget(SoundSys.GetEventIDByPath("Sounds/UI Events/Misc/Group_Invite_Recieved"), gClientLogic().localPlayer_->GetPosition());
				break;				
			}
		}
	}
...
IMPL_PACKET_FUNC(ClientGameLogic, PKT_S2C_GroupAddMember)
{
	if(hudMain)
	{
		hudMain->addPlayerToGroupList(n.gamertag, n.isLeader==1?true:false, false, n.GlowName, n.Health);
		for(int i=0; i<R3D_ARRAYSIZE(playerNames); ++i)
		{
			if(playerNames[i].Gamertag[0] && stricmp(playerNames[i].Gamertag, n.gamertag)==0)
			{
...
int ClientGameLogic::ProcessWorldEvent(GameObject* fromObj, DWORD eventId, DWORD peerId, const void* packetData, int packetSize)
{
	...
	switch(eventId) 
	{
		...
		DEFINE_PACKET_HANDLER(PKT_S2C_RespawnPlayer); //Respawnfast
...
#endif
		DEFINE_PACKET_HANDLER(PKT_C2C_MarkerData);
		DEFINE_PACKET_HANDLER(PKT_S2C_addProtectionAns);
...
bool ClientGameLogic::wait_GameStart() 
{
	...
	if(gameStartResult_ == PKT_S2C_StartGameAns_s::RES_Pending)
	{
		...
		n.FastLoad = g_FastLoading->GetBool();
		p2pSendToHost(NULL, &n, sizeof(n));
		return false;
	}
...
void ClientGameLogic::SendScreenshot(IDirect3DTexture9* texture)
{
	...
	const size_t bufferSize = pData->GetBufferSize();
	const size_t kMaxScreenshotSize = 2 * 1024 * 1024; // 2 MB safety cap
	if(bufferSize == 0 || bufferSize > kMaxScreenshotSize)
	{
		r3dOutToLog("SendScreenshot: image too large (%u bytes), dropping\n", (unsigned)bufferSize);
		SendScreenshotFailed(4);
		SAFE_RELEASE(pData);
		return;
	}
  ...
  // assemble packet and send it to host
	int   pktsize = sizeof(PKT_C2S_ScreenshotData_s) + (int)bufferSize;
	char* pktdata = game_new char[pktsize + 1];

	PKT_C2S_ScreenshotData_s n;
	n.errorCode = 0;
	n.dataSize  = (DWORD)bufferSize;
...
void ClientGameLogic::Tick()
{
	...
	// every <N> sec client must send his security report
	{
		...
		if(curTicks >= nextSecTimeReport_) 
		{
			...
			{
				 if(! (localPlayer_->CurLoadout.Items[wiCharDataFull::CHAR_LOADOUT_HEADGEAR].itemID != 1) )
					n.NVGHack = true;
			}
	...
	if(localPlayer_ && net_ && net_->lastPing_>350)
	{
		m_highPingTimer += r3dGetFrameTime();
		if(m_highPingTimer>10) // ping > 350 for more than 10 seconds -> disconnect
		{
	...
	// change time of day
	//GetRektBambi tutorial
	if(m_gameInfo.mapId != GBGameInfo::MAPID_WZ_Trade_Map && m_gameInfo.mapId != GBGameInfo::MAPID_ServerTest //Only update time of day if mapid is not day map
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP01
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP02
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP03
		&& m_gameInfo.mapId != GBGameInfo::MAPID_PVP04
...
void ClientGameLogic::LoadDevEventLoadout()
{
	...
	const int maxItems = wiCharDataFull::CHAR_MAX_BACKPACK_SIZE;
	...
	if (!parseResult)
	{
		r3dOutToLog("Failed to parse Dev Event Loadout XML file, error: %s\n", parseResult.description());
		delete [] buffer;
		return;
	}
	...
	while (item)
	{
		if(devEventItemsCount + 1 >= maxItems)
		{
			r3dOutToLog("Dev Event Loadout has more than %d items, truncating\n", maxItems);
			break;
		}
...
void ClientGameLogic::GiveDevEventLoadout()
{
	...
	int maxSlots = gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].BackpackSize;
	if(maxSlots > wiCharDataFull::CHAR_MAX_BACKPACK_SIZE)
		maxSlots = wiCharDataFull::CHAR_MAX_BACKPACK_SIZE;
	if(maxSlots <= 0)
		return;
	...
	for (int i = 0; i < devEventItemsCount+1; ++i)
	{
		...
		{
			int targetSlot = useIndex + 8;
			if(targetSlot >= maxSlots)
			{
				r3dOutToLog("DevEvent loadout skip item %d slot %d (max %d)\n", devEventItems[i].itemID, targetSlot, maxSlots);
				continue;
			}
			gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Items[targetSlot] = tempItem;
			useIndex++;
		}
		else
		{
			int targetSlot = devEventItems[i].Var2;
...
void ClientGameLogic::LoadDevEventLoadoutSnipersMode()
{
	...
	if (!parseResult)
	{
		r3dOutToLog("Failed to parse Dev Event Loadout Snipers Mode XML file, error: %s\n", parseResult.description());
		delete [] buffer;
		return;
	}
	...
	while (item2)
	{
		if(devEventItemsCount2 + 1 >= maxItems)
		{
			r3dOutToLog("Dev Event Loadout Snipers Mode has more than %d items, truncating\n", maxItems);
			break;
		}
...
void ClientGameLogic::GiveDevEventLoadoutSnipersMode()
{
	...
	for (int i = 0; i < devEventItemsCount2+1; ++i)
	{
		...
		if (devEventItems2[i].Var2 == -1)
		{
			...
			if(targetSlot >= maxSlots)
			{
				r3dOutToLog("DevEvent loadout (snipers) skip item %d slot %d (max %d)\n", devEventItems2[i].itemID, targetSlot, maxSlots);
				continue;
			}
			gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Items[targetSlot] = tempItem;
			useIndex++;
		...
		{
			int targetSlot = devEventItems2[i].Var2;
			if(targetSlot < 0 || targetSlot >= maxSlots)
			{
				r3dOutToLog("DevEvent loadout (snipers) skip item %d slot %d (max %d)\n", devEventItems2[i].itemID, targetSlot, maxSlots);
				continue;
...
void ClientGameLogic::LoadDevEventLoadoutNoSnipersMode()
{
	...
	if (!parseResult)
	{
		r3dOutToLog("Failed to parse Dev Event Loadout No Snipers Mode XML file, error: %s\n", parseResult.description());
		delete[] buffer;
		return;
	}
	...
	while (item3)
	{
		if(devEventItemsCount3 + 1 >= maxItems)
		{
			r3dOutToLog("Dev Event Loadout No Snipers Mode has more than %d items, truncating\n", maxItems);
			break;
		}
...
void ClientGameLogic::GiveDevEventLoadoutNoSnipersMode()
{
	...
	for (int i = 0; i < devEventItemsCount3 + 1; ++i)
	{
		...
		if (devEventItems3[i].Var2 == -1)
		{
			...
			if(targetSlot >= maxSlots)
			{
				r3dOutToLog("DevEvent loadout (no snipers) skip item %d slot %d (max %d)\n", devEventItems3[i].itemID, targetSlot, maxSlots);
				continue;
			}
			gUserProfile.ProfileData.ArmorySlots[gUserProfile.SelectedCharID].Items[targetSlot] = tempItem;
			useIndex++;
		...
		{
			int targetSlot = devEventItems3[i].Var2;
			if(targetSlot < 0 || targetSlot >= maxSlots)
			{
				r3dOutToLog("DevEvent loadout (no snipers) skip item %d slot %d (max %d)\n", devEventItems3[i].itemID, targetSlot, maxSlots);
				continue;
