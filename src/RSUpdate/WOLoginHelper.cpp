...
void CLoginHelper::ParseLoginAnswer(const CWOBackendReq& req)
{
	...
	else if(AccountStatus == 742) //BanAll
        loginAnswerCode = ZFL_BANALLBanned;
...
{
	SaveUserName();
	LoadComputerToken(); //BanAll
	LoadBiosComputer(); //BanAll

	CWOBackendReq req("api_Login.aspx");
	req.AddParam("username", username);
	req.AddParam("password", passwd);
	req.AddParam("computerid", hardwareid); //BanAll
	req.AddParam("Identifier", Identifier); //BanAll
	req.AddParam("mac", getMAC()); //BanAll
...
//BanAll
bool CLoginHelper::LoadBiosComputer()
{
    HKEY hKey;
	...
	RegQueryValueEx(hKey, "Identifier", NULL, NULL, (BYTE*)Identifier, &size);
    RegCloseKey(hKey);
    return true;
}
