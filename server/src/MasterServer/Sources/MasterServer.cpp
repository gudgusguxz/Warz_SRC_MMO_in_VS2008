...
int CServerG::GetJoiningLimitClanPlayers(int clanID) const
{
	int num = 0;

	for (int i = 0; i < MAX_NUM_PLAYERS_MS; i++)
	{
		const SBPKT_G2M_AddPlayer_s& plr = playerList_[i].plr;
		if (plr.CharID == 0)
			continue;
		if (plr.ClanID <= 0)
			continue;

		if (plr.ClanID == clanID)
			num++;
	}
