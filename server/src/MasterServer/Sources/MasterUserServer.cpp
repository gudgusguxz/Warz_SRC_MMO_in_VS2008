...
void CMasterUserServer::PrintStats()
{
  ...
  int numGames     = 0;
  int numGamesH[1] = {0}; // hosted games per region
  int numGamesR[1] = {0}; // renged games per region
  int numHoldsR[1] = {0}; // strongholds per region
  int numCCU       = 0;
  int maxCCU       = 0;
  ...
  int regionsToLog[1] = {GBNET_REGION_US_West/*, GBNET_REGION_Europe, GBNET_REGION_Russia, GBNET_REGION_SouthAmerica*/};
...
void CMasterUserServer::OnGBPKT_C2M_JoinGameReq(DWORD peerId, const GBPKT_C2M_JoinGameReq_s& n)
{
  ...
  do 
  {
    ...
    if(game) {
      DoJoinGame(game, n.CustomerID, n.pwd, ans, n.clanID);
      break;
    }
	...
	int clanLimit = game->GetJoiningLimitClanPlayers(n.clanID);
	if (gameStatus == 2 && game->info_.ginfo.flags & GBGameInfo::SFLAGS_ClanLimit)
	{
		if (clanLimit >= game->info_.ginfo.ClanLimitMax)
		{
			ans.result = GBPKT_M2C_JoinGameAns_s::rFullPlayerClanLimit;
			break;
		}
	}
...
void CMasterUserServer::DoJoinGame(CServerG* game, DWORD CustomerID, const char* pwd, GBPKT_M2C_JoinGameAns_s& ans, int clanID)
{
  r3d_assert(game);
  ...
  int clanLimit = game->GetJoiningLimitClanPlayers(clanID);
  if (game->info_.ginfo.flags & GBGameInfo::SFLAGS_ClanLimit)
  {
	  if (clanLimit >= game->info_.ginfo.ClanLimitMax)
	  {
		  ans.result = GBPKT_M2C_JoinGameAns_s::rFullPlayerClanLimit;
