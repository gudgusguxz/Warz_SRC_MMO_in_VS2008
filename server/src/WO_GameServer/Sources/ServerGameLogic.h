...
private:// Snipers Mode
	wiInventoryItem devEventItems2[72];
	int				devEventItemsCount2;
	int				devEventBackpackId2;

	void GiveDevEventLoadoutSnipersMode(DWORD peerId);
private:// No Snipers Mode
	wiInventoryItem devEventItems3[72];
	int				devEventItemsCount3;
	int				devEventBackpackId3;

	void GiveDevEventLoadoutNoSnipersMode(DWORD peerId);
public:
	void PreloadDevEventLoadout();
	void PreloadDevEventLoadoutSnipersMode();
	void PreloadDevEventLoadoutNoSnipersMode();
  ...
  public:
	...
	obj_ServerPlayer* CreateNewPlayer(DWORD peerId, const r3dPoint3D& spawnPos, float spawnDir, bool needsSpawnProtection, bool isFastLoad);
	...
	float		m_LastConnectionRep;//AlexRedd:: Last Connection
	...
	void		  GetSpawnPositionAfterDeathWarZTH(const r3dPoint3D& GamePos, r3dPoint3D* pos, float* dir);

	void		addProtectionAns(obj_ServerPlayer* plr);
	...
	void		RespawnPlayer(obj_ServerPlayer* plr); // Respawnfast 
...
#endif
	 DEFINE_PACKET_FUNC(PKT_C2C_MarkerData);
	...
	int		  Cmd_BanChat(obj_ServerPlayer* plr, const char* cmd);//AlexRedd:: Ban chat
	...
	int		  Cmd_VehicleSpawn2(obj_ServerPlayer* plr, const char* cmd);
#endif
	...
	void		RewardKillPlayer(obj_ServerPlayer* sourcePlr, obj_ServerPlayer* targetPlr, STORE_CATEGORIES dmgCat);//AlexRedd:: kill rewards
...
obj_ServerPlayer* GetPlayer(int playerIdx)
 {
   ...
   return plrToPeer_[playerIdx]->player;
	...
	void		LogTradeInfo(DWORD peerId, int disconnect, int ItemID, int Quantity, int GameDollar, const char* fmt, ...); //TradeGD
