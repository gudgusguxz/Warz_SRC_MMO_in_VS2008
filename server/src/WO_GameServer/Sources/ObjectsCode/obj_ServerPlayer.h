...
class obj_ServerPlayer : public GameObject, INetworkHelper
{
  public:
	...
	bool		isHighPing; // Alexredd:: Hight Ping
	int			Trade_GameDollar; //TradGD
 ...
 //
	...
	void		DoDeath();
	void		DoRespawn(const r3dPoint3D& pos, float dir, float spawnProtection); // Respawnfast
	...
	int		savedGamePoints_;
	void		SetProfile(const CServerUserProfile& in_profile);
	int			numKillWithoutDying;//AlexRedd:: Kill rewards
	...
	void		OnExchangeCallback(const CJobExchangeAdd* job);
	...
	void		SellItemWithGD(int slot, bool repailAllRequest);
	void		SellItemWithGC(int slot, bool repailAllRequest);
...
float lastPlayerFire;
float lastPlayerPickup;
float lastPlayerUsedMed;
float lastPlayerMoveRel;
float lastPlayerUpdateMem;
float lastUpdateMem;
float lastMedCooldownTimeTier;
...
int m_unknow_0;
	...
	int			m_AnglingStatus;
	uint32_t	m_FishrodID;
	uint32_t	m_BaitID;
	int			m_BaitQty;
	uint32_t	m_Fishreward;
	float		m_LastUpdateStatus;
...
public:
	...
	void		OnNetPacket(const PKT_C2C_PlayerSwearing_s& n);
	...
	void		OnNetPacket(const PKT_C2S_BackpackDropAll_s& n);//AlexRedd:: drop all	
	...
	void		OnNetPacket(const PKT_C2S_Temp_Damage_s& n);
	...
	void		OnNetPacket(const PKT_C2C_TradeGD_s& n); //TradeGD
	void		OnNetPacket(const PKT_C2S_ExchangeItem_s& n);
	void		OnNetPacket(const PKT_C2S_CraftingNewItem_s& n);
	void		OnNetPacket(const PKT_C2S_CraftingNewItemAdd_s& n);
	void		OnNetPacket(const PKT_C2S_OpenBoxReadyPlay_s& n);

	//Respawnfast
	void		OnNetPacket(const PKT_C2S_SetRespawnData_s& n); 
	...
	void		OnNetPacket(const PKT_C2S_AnglingCancel_s& n);
