...
BOOL obj_ServerGrenade::Update()
{
	...
	{
		if(m_ItemID == WeaponConfig::ITEMID_ZombieGrenade)
			OnSpawn();
		else
		OnExplode();
		return FALSE;
...
void obj_ServerGrenade::OnSpawn()
{
	// if owner disappeared, do nothing.
	GameObject* owner = GameWorld().GetObject(ownerID);
	if(!owner)
	...
	obj_ZombieSpawn* spawnObject = obj_ZombieSpawn::GetClosestToPosition(GetPosition());
	r3dVector position = GetPosition();

	int zombieCount = u_random(5) +1;

	for (int i = 0; i < zombieCount; ++i)
	{	
		if (zombieCount > 1)
		{
			position.x = u_GetRandom(GetPosition().x - 5.0f, GetPosition().x + 5.0f);
			position.z = u_GetRandom(GetPosition().z - 5.0f, GetPosition().z + 5.0f);
