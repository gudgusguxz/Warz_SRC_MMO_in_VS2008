...
AUTOREGISTER_CLASS(obj_Zombie);
	...
	float		_zai_AttackDamage					= 0.0f; //¥“‡¡®´Õ∫∫’È
	float		_zai_SuperZombieSuperAttackDamage	= 0.0f;	//¥“‡¡®∫Õ  // 1.25 * _zai_AttackDamage
	int		_zai_DebugAI							= 0;
#endif
	...
	const float _zstat_MaxHealthSuperZombie2 = 2800;
...
// 		}
	...
	else if( IsSuperZombie2() )
		{
		ZombieHealth	= _zstat_MaxHealthSuperZombie2;
		FastZombie		= true;
		WalkSpeed		= 1.9f;
		RunSpeed		= 5.4f;
	}
	...
	CanSprint = IsSuperZombie() ? false : ( IsSuperZombie2() ? false : spawnObject->m_SprintersSpawnPerc >= u_GetRandom(0.0f, 1.0f) );
...
int obj_Zombie::CheckMoveWatchdog()
{
	...
	if(navAgent->m_status != AutodeskNavAgent::Moving)
		return 2;
	if( (!IsSuperZombie()) || (!IsSuperZombie2())  && staggerTime > 0)
		return 1;
...
{
	const float eyeHeight = ( IsSuperZombie() || IsSuperZombie2() ) ? 2.4f : 1.6f;
#ifdef ZOMBIE_PERFORMANCE_TESTING
	float startTime = r3dGetTime();
...
bool obj_Zombie::SenseGrenadeSound(const obj_ServerGrenade* grenade, bool isExplosion)
{
	...
	if( isExplosion )
	{
		switch(grenade->m_ItemID)
		{
			...
			case WeaponConfig::ITEMID_ZombieGrenade:
...
bool obj_Zombie::StartAttack(const GameObject* trg)
{
	...
	SetNavAgentSpeed((IsSuperZombie() || IsSuperZombie2() || staggerTime < 0) ? RunSpeed : 0.0f);
...
bool obj_Zombie::CanAttack(const GameObject* trg)
{
	if(trg)
	{
		...
		float attackRadius = ( IsSuperZombie() || IsSuperZombie2() )?_zai_SZAttackRadius:_zai_AttackRadius;
...
BOOL obj_Zombie::Update()
{
	...
	if(ZombieState == EZombieStates::ZState_Dead)
	{
		...
		if(curTime > StateStartTime + 5.0f)//60.0f  AlexRedd:: now dead zombie faster delete
			setActiveFlag(0);
		return TRUE;
	}
	...
	switch(ZombieState)
	{
		...
		case EZombieStates::ZState_CallForHelp:
		{
			...
			if( ( (IsSuperZombie() || IsSuperZombie2() ) && _zai_SZCallForHelpTime < curTime - StateStartTime ) ||
				( (!IsSuperZombie()) || (!IsSuperZombie2())  && _zai_CallForHelpTime < curTime - StateStartTime ) )
			{
				GameObject* trg = GameWorld().GetObject( hardObjLock );
				SetTarget( invalidGameObjectID );
		...
		case EZombieStates::ZState_Idle:
		{
			...
			bool doPatrol = !ZombieDisabled && u_GetRandom() < (( IsSuperZombie() || IsSuperZombie2() ) ? _zai_SuperZombieIdleStatePatrolPerc :_zai_IdleStatePatrolPerc);
		...
		case EZombieStates::ZState_Attack:
		{
			...
			if (gameObj->isObjType(OBJTYPE_Vehicle))
			{
				...
				if( ( (IsSuperZombie() || IsSuperZombie2()) && attackCounter != 2 && attackTimer >= _zai_SuperZombieAttackTimer ) ||
					( (IsSuperZombie() || IsSuperZombie2()) && attackCounter == 2 && attackTimer >= _zai_SuperZombieSuperAttackTimer ) ||
					( (!IsSuperZombie()) || (!IsSuperZombie2())  && attackTimer >= _zai_AttackTimer ) )
				{
					attackTimer = 0;
					...
					else
					{
						...
						float damage = ( IsSuperZombie() || IsSuperZombie2() )? _zai_SuperZombieSuperAttackDamage : _zai_AttackDamage;
						
						vehicle->ApplyDamage(this, damage, storecat_MELEE);

						if( (IsSuperZombie() || IsSuperZombie2()) && attackCounter == 2 )
						{
							superAttackDir++;
							superAttackDir %= 2;
						}
		...
		case EZombieStates::ZState_MovingAttack:
		{
			...
			if( !IsSuperZombie() || !IsSuperZombie2() || ZombieState ==  EZombieStates::ZState_BarricadeAttack )
			{
				if(staggerTime > 0)
				{
					attackTimer   = 0;
...
DefaultPacket* obj_Zombie::NetGetCreatePacket(int* out_size)
{
	...
	n.SZombieHealth = IsSuperZombie()||IsSuperZombie2() ? ZombieHealth : 0.0f;//AlexRedd:: boss health
...
//	{
//		int BossDropItemQty = 1;
//		if (IsSuperZombie()) BossDropItemQty = u_random(4) +10; //BossDrop
//		//if (IsSuperZombieForced) BossDropItemQty = 12;
//		for (int Qty = 0; Qty < BossDropItemQty; Qty++)
//		{
...
void obj_Zombie::DoDeath(bool fakeDeath)
{
	...
	if(!fakeDeath && spawnObject->lootBoxCfg)
	{
		...
		m_LootBoxConfig = const_cast<LootBoxConfig*>(g_pWeaponArmory->getLootBoxConfig(302309));

		int HowManyToDrop = 1; //ZombieDrop
		if( IsSuperZombie() || IsSuperZombie2() ) 
			HowManyToDrop = u_random(4) +5; //BossDrop 
		...
		for(int Amount = 0; Amount < HowManyToDrop; Amount++)
		{
			...
			if(HeroItemID == 20227)
				...
				wi = RollItem(spawnObject->lootBoxCfg, 0);
...
bool obj_Zombie::ApplyDamage(GameObject* fromObj, float damage, int bodyPart, STORE_CATEGORIES damageSource)
{
	...
	if( (!IsSuperZombie()) && (!IsSuperZombie2()) )
	{
		...
		{
			if(bodyPart != 1) // only hitting head will lower zombie's health (except grenades)
			{
	...
	if((IsSuperZombie() && IsSuperZombie2())&& damageSource==storecat_MELEE)
		damage = 0;
...
bool obj_Zombie::IsSuperZombie2()
{
	return HeroItemID == g_SuperZombieItemID2;
}
...
void obj_Zombie::PassiveHeal()
{
	if( ZombieHealth >= (IsSuperZombie() ? _zstat_MaxHealthSuperZombie : _zstat_MaxHealth) )
		return;
	if( ZombieHealth >= (IsSuperZombie2() ? _zstat_MaxHealthSuperZombie2 : _zstat_MaxHealth) )
		return;
	...
	if( curTime > HealTimer )
	{
		...
		if( IsSuperZombie2() )
		{
			ZombieHealth = R3D_CLAMP( ZombieHealth + _zstat_MaxHealthSuperZombie2 * _zai_SZHealRate , ZombieHealth, _zstat_MaxHealthSuperZombie2 ); 
			ResetPassiveHealTimer();
		}
	}
}
