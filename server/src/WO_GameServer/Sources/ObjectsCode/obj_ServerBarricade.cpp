...
const static int BARRICADE_EXPIRE_TIME = 5 * 60; // barricade will expire in 10 นาที //โล่
const static int STRONGHOLD_EXPIRE_TIME = 5 * 60; // stronghold items will expire 10 นาที //โล่
const static int DEV_EVENT_EXPIRE_TIME = 5 * 60; // dev event items will expire in 10 นาที //โล่
...
BOOL obj_ServerBarricade::OnCreate()
{
	...
	else if(m_ItemID == WeaponConfig::ITEMID_WoodShieldBarricade || m_ItemID==WeaponConfig::ITEMID_WoodShieldBarricadeZB)
	{
		...
		|| m_ItemID==WeaponConfig::ITEMID_RiotShieldTM|| m_ItemID==WeaponConfig::ITEMID_RiotShieldJumper 
		|| m_ItemID==WeaponConfig::ITEMID_RiotShieldGalax)
	{
