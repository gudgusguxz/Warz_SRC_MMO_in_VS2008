...
void ServerGameLogic::ApiPlayerUpdateChar(obj_ServerPlayer* plr, bool disconnectAfter)
{
	...
	job->GC_Diff	= plr->profile_.ProfileData.GamePoints - plr->savedGamePoints_;
	...
	plr->savedGamePoints_ = plr->profile_.ProfileData.GamePoints;
}
...
void ServerGameLogic::LogTradeInfo(DWORD peerId, int disconnect, int ItemID, int Quantity, int GameDollar, const char* fmt, ...) //TradeGD
{
	char buf[4096];
	va_list ap;
	va_start(ap, fmt);
	...
	CJobAddLogTradeInfo* job = new CJobAddLogTradeInfo();
...
int ServerGameLogic::getReputationKillEffect(int repFromPlr, int repKilledPlr)
{
	...
	if(repFromPlr > -20 && repFromPlr < 20)
	{
		if(repKilledPlr >= 2000000)
			...
			repFromPlr -= 2;
//
		else if(repKilledPlr <= -2400000)
			repFromPlr += 2600;
		else if(repKilledPlr <= -1000000)
			repFromPlr += 1950;
		else if(repKilledPlr <= -600000)
			repFromPlr += 1300;
		else if(repKilledPlr <= -200000)
			repFromPlr += 650;
		else if(repKilledPlr <= -150000)
			repFromPlr += 420;
		else if(repKilledPlr <= -90000)
			repFromPlr += 250;
		else if(repKilledPlr <= -30000)
			repFromPlr += 100;
		else if(repKilledPlr <= -1000)
			repFromPlr += 20;
	...
	// BANDIT LOGIC (once you are in bandit section, you cannot come up to be neutral) (สายโจร)
...
void ServerGameLogic::RewardKillPlayer(obj_ServerPlayer* fromPlr, obj_ServerPlayer* targetPlr, STORE_CATEGORIES dmgCat)
{
	...
	else if (fromPlr->numKillWithoutDying == 50)
	{
		...
		//LogCheat(fromPlr->peerId_, PKT_S2C_CheatWarning_s::CHEAT_Kills, true, "HighKills 50"); //TOEiX2TEST
	}
	else if (fromPlr->numKillWithoutDying == 100)
	{
		AddPlayerReward(fromPlr, RWD_Kill100NotDying);
		//LogCheat(fromPlr->peerId_, PKT_S2C_CheatWarning_s::CHEAT_Kills, true, "HighKills 100"); //TOEiX2TEST
	}
...
bool ServerGameLogic::ApplyDamageToPlayer(GameObject* fromObj, obj_ServerPlayer* targetPlr, const r3dPoint3D& dmgPos, float damage, int bodyBone, int bodyPart, bool force_damage, STORE_CATEGORIES damageSource, uint32_t dmgItemID, int airState, bool canApplyBleeding)
{
	...
	if(targetPlr->groupID!= 0 && IsServerPlayer(fromObj) && IsServerPlayer(fromObj)->groupID==targetPlr->groupID && damageSource!=storecat_GRENADE 
		...
		return false;
	// no damage in clan
	if(targetPlr->loadout_->ClanID !=0 && IsServerPlayer(fromObj) && IsServerPlayer(fromObj)->loadout_->ClanID == targetPlr->loadout_->ClanID && damageSource!=storecat_GRENADE
		&& fromObj!=targetPlr
		)
		return false;
	if (ginfo_.flags & GBGameInfo::SFLAGS_ClanLimit)
	{
		if (targetPlr->loadout_->ClanID != 0 && IsServerPlayer(fromObj) && IsServerPlayer(fromObj)->loadout_->ClanID == targetPlr->loadout_->ClanID && fromObj != targetPlr)
			return false;
	}
	...
	if(fromObj->Class->Name == "obj_Zombie")
		...
		if(ginfo_.channel==4) //Premium PVE
			return false;
	}
...
void ServerGameLogic::AddDirectPlayerReward(obj_ServerPlayer* plr, const wiStatsTracking& in_rwd, const char* rewardName)
{
	...
	//r3dOutToLog("reward: %s got %dxp %dgp %dgd RWD_%s\n", plr->userName, xp, gp, gd, rewardName ? rewardName : ""); //TEST
...
obj_ServerPlayer* ServerGameLogic::CreateNewPlayer(DWORD peerId, const r3dPoint3D& spawnPos, float spawnDir, bool needsSpawnProtection, bool isFastLoad)
{
	...
	if(gServerLogic.ginfo_.mapId==GBGameInfo::MAPID_WZ_California)
		...
		if (ginfo_.channel == 6/* && ginfo_.gameServerId != 22000*/)// 22000 = no drop (sinch point client/server)
		{
			wipeInventory = true;			
	...
	if(needsSpawnProtection)
		plr->m_SpawnProtectedUntil = r3dGetTime() + 30.0f; //ปรับเวลาโพเทค
	plr->peerId_      = peerId;
	...
	if(!(plr->profile_.ProfileData.isDevAccount & wiUserProfile::DAA_HIDDEN))
	{
		...
		n.gamertagcolor = plr->loadout_->GamertagColor;
		...
		for(int i=0; i<MAX_PEERS_COUNT; i++) {
			if(peers_[i].status_ >= PEER_PLAYING && peers_[i].player) {
				if(!(peers_[i].player->profile_.ProfileData.isDevAccount & wiUserProfile::DAA_HIDDEN))
				{
					...
					n.gamertagcolor = peers_[i].player->loadout_->GamertagColor;
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2S_StartGameReq)
{
	...
	if (ginfo_.flags & GBGameInfo::SFLAGS_ClanLimit)
	{
		...
		for (int i = 0; i < MAX_PEERS_COUNT; ++i)
		{
			...
			{
				if (peerInfo.player != NULL && peerInfo.player->loadout_->ClanID != 0)
				{
					if (peerInfo.player->loadout_->ClanID == loadout.ClanID)
					{
						clanlimitPlrs++;
		...
		if (found)
		{
			if (clanlimitPlrs >= ginfo_.ClanLimitMax)
			{
				PKT_S2C_StartGameAns_s n;
				n.result = PKT_S2C_StartGameAns_s::RES_Failed;
				p2pSendRawToPeer(peerId, &n, sizeof(n));
...
void ServerGameLogic::GetStartSpawnPosition(const wiCharDataFull& loadout, r3dPoint3D* pos, float* dir, bool& needsSpawnProtection)
{
	...
	if(loadout.GameMapId && loadout.Alive == 1 || loadout.Alive == 2) //ดิสย้ายจุด
	{
		needsSpawnProtection = true;
...
void ServerGameLogic::GetSpawnPositionAfterDeath2(const r3dPoint3D& GamePos, r3dPoint3D* pos, float* dir)
{
	if(gCPMgr.numControlPoints_ == 0)
	{
		r3dError("!!!!!!!!!!!! THERE IS NO CONTROL POINT 2 !!!!!!!\n");
		*pos = r3dPoint3D(0, 0, 0);
		*dir = 0;
		return;
	}
...
void ServerGameLogic::GetSpawnPositionAfterDeathWarZTH(const r3dPoint3D& GamePos, r3dPoint3D* pos, float* dir)
{
	if(gCPMgr.numControlPoints_ == 0)
	{
		r3dOutToLog("!!!!!!!!!!!! THERE IS NO CONTROL POINT WARZTH !!!!!!!\n");
		*pos = r3dPoint3D(0, 0, 0);
		*dir = 0;
		return;
	}
...
int ServerGameLogic::ProcessChatCommand(obj_ServerPlayer* plr, const char* cmd)
{
	...
	if(strncmp(cmd, "/chatban", 4) == 0 && plr->profile_.ProfileData.isDevAccount & wiUserProfile::DAA_BAN)//AlexRedd:: Ban chat
	...
	if (strncmp(cmd, "/vspawn", 7) == 0 && plr->profile_.ProfileData.isDevAccount)
		...
		{
			if (ginfo_.mapId==GBGameInfo::MAPID_WZ_Colorado/* || ginfo_.mapId==GBGameInfo::MAPID_WZ_Colorado_PVE || ginfo_.mapId==GBGameInfo::MAPID_WZ_pvp10*/)
				return Cmd_VehicleSpawn2(plr, cmd);
		}
#endif
...
int ServerGameLogic::Cmd_Ammo(obj_ServerPlayer* plr, const char* cmd)
{
    ...
    if(wpn && wpn->getCategory() != storecat_MELEE)
    {
        ...
        wi.quantity = 10; //set how many mags per command you want!
	...
	for(int i=0; i<curPlayers_; ++i)
	{
		...
		if(stricmp(pl->userName, plrName)==0)
		{
			...
			//{
				...
				if (pl->profile_.ProfileData.isDevAccount) // Kick it all in.
				{
				}
				else
...
int ServerGameLogic::Cmd_VehicleSpawn2(obj_ServerPlayer* plr, const char* cmd)
{
	char buf[128];
	int vehicleType = 0;
	...
	wi.itemID   = 800124;
	wi.quantity = 1;	
	int numNotInBackpack = plr->BackpackRemoveItem( wi );
	if( numNotInBackpack == 0 )
	{
		...
		r3dOutToLog("[%s] Player has spawned a vehicle.\n", plr->userName);
	}
...
int ServerGameLogic::Cmd_ZombieSpawn(obj_ServerPlayer* plr, const char* cmd, bool isSuperZombie2)
{
	char buf[128];
	int zombieCount = 0;
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2S_Temp_Damage)
{
	...
	ApplyDamage(fromObj, target, n.explosion_pos, damage, true, wc->category, wc->m_itemID, false);
}
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2C_ChatMessage)
{
	...
	const int   CHAT_NUMBER_TO_SPAM    = 9999;		// number of messages below delay time to be considered spam
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2C_MarkerData)
{
	if (n.id < 0 || n.id > 3) {
		DisconnectPeer(peerId, true, "invalid PKT_C2C_MarkerData #1");
		return;
	}

	if (fromObj == NULL || !IsServerPlayer(fromObj)) {
		DisconnectPeer(peerId, true, "invalid PKT_C2C_MarkerData #2");
		return;
	}
...
IMPL_PACKET_FUNC(ServerGameLogic, PKT_C2S_SecurityRep)
{
	...
	if(peer.player && peer.player->profile_.ProfileData.isDevAccount == 0 && gServerLogic.ginfo_.channel!=1 && gServerLogic.ginfo_.channel!=2 && gServerLogic.ginfo_.channel!=3 && gServerLogic.ginfo_.channel!=4 && gServerLogic.ginfo_.channel!=5 && gServerLogic.ginfo_.channel!=6 && gServerLogic.ginfo_.channel!=7 )
	{
...
void ServerGameLogic::joinPlayerToGroup(obj_ServerPlayer* plr, uint32_t groupID)
{
	...
	n2.GlowName = plr->loadout_->GamertagColor; //เปลี่ยนชื่อตัวแปรเป็น ของตัวเองถ้าไม่ใช่ GlowName
	...
	for(int i=0; i<curPlayers_; ++i)
	{
		...
		if(pl->groupID == groupID)
		{
			...
			n.GlowName = pl->loadout_->GamertagColor; //เปลี่ยนชื่อตัวแปรเป็น ของตัวเองถ้าไม่ใช่ GlowName
...
void ServerGameLogic::createNewPlayerGroup(obj_ServerPlayer* leader, obj_ServerPlayer* plr, uint32_t groupID)
{
	...
	n.GlowName = leader->loadout_->GamertagColor; //เปลี่ยนชื่อตัวแปรเป็น ของตัวเองถ้าไม่ใช่ GlowName
		...
		for(int i=0; i<curPlayers_; ++i)
		{
			...
			if(pl && pl->groupID == groupID)
			{
				if(reassignLeader)
				{
					...
					r3dscpy(n2.gamertag, pl->userName);
					n2.GlowName = pl->loadout_->GamertagColor; //เปลี่ยนชื่อตัวแปรเป็น ของตัวเองถ้าไม่ใช่ GlowName
...
void ServerGameLogic::RespawnPlayer(obj_ServerPlayer* plr) //Respawnfast
{
	//Get random respawn point
	r3dPoint3D pos;
	float dir;
	if (gServerLogic.ginfo_.flags & GBGameInfo::SFLAGS_Respawnfast)
	{
		for (int i = 0; i < gCPMgr.numControlPoints_; i++)
		{
			int idx = u_random(i);// get random control point
			const BasePlayerSpawnPoint* spawn = gCPMgr.controlPoints_[idx];
			for (int j = 0; j < spawn->m_NumSpawnPoints; j++)
			{
	...
	float respawnProtection = 30.0f; //ปรับเวลาโพเทค *ตายเกิด
